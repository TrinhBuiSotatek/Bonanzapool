...
All checkpoint files live in `.claude/skills/qc-func-tc-design-exbot/process-logging/<UC-ID>/`. Create the folder lazily — when the first checkpoint is written. One subfolder per UC so the skill can run for multiple UCs concurrently without conflict.
...
| `02_designed_tcs_<variant>.md`          | Phase 1                                     | **Source of truth for the designed test cases — ONE file per platform variant.** Atomic snapshot of the FULL TC list + RTM for that variant, written BEFORE the variant's deliverable `.md` is written (Step 3.5 of `generate-test-cases.md`, Step 5.3 of `update-test-cases.md`). Phase 2 Step 0 reads this when auto-recovering a partial / mismatched final md (overwrites the same-version final md from the scratch). Same content format as the final deliverable md (prelude + operation-group / typed-section headers + TC tables). This skill is logic-only, so there is exactly one file with the literal token `logic` in place of a variant name, for both generate and update runs. |
...
```markdown
# qc-func-tc-design-exbot progress — <UC-ID>
...
**FUNC total:** <a>   <!-- Functional logic cases -->
**INTG+NFR total:** <b>   <!-- Integration/State + Non-functional logic cases -->
...
| Operation group | Total | FUNC | INTG+NFR |
|---|---|---|---|
| I. <operation name> | <n1> | <a1> | <b1> |
| II. <operation name> | <n2> | <a2> | <b2> |
| ... | ... | ... | ... |
...
**Notes on the schema:**
- `Total test cases designed`, `FUNC total`, `INTG+NFR total` are explicit, separately-parseable fields on their own lines (Phase 2 Step 0 parses each as a discrete integer). The binding verification check is the Total only; the FUNC / INTG+NFR split is informational.
...
1. Check `.claude/skills/qc-func-tc-design-exbot/process-logging/<UC-ID>/progress.md`.
...
4. **Default case:** Ask the user (ONE message, blocking):
  ```
  Found a checkpoint from a previous run for UC <UC-ID>:
  - Run ID: <run_id>
  - Workflow: <generate-test-cases | update-test-cases>
  - Started at: <started_at>
  - Completed: Phase <last_phase_done> (<phase friendly name>)

  You can:
  1. **Continue** — resume from Phase <next_phase>
  2. **Restart** — start over (delete all previous checkpoints)
  ```
...
6. If user picks **Restart**:
  - Delete `.claude/skills/qc-func-tc-design-exbot/process-logging/<UC-ID>/` folder entirely.
...
| --- | --- |
| 1 (fresh) | All input files re-resolved from `path-registry.md` — UC review report, scenarios (optional), `project-context-master.md`, `common-logic-technical.md`, `logic-technical.md`, `Testcase-refer-en.md`. For `update-test-cases`: also the baseline TC `.md` file(s), the old audited UC (the one used by the baseline), the new audited UC, and any user feedback in the current prompt. |
