...
| **D1 Database** | Cơ sở dữ liệu SQLite sharded của Cloudflare để lưu trữ trạng thái hiện tại (hot state), lịch sử rebalance, circuit breakers, và API keys mã hóa. | Backend Services | Database Storage | Phase A1 dùng 1 DB shard. Phase C chia làm 16 DB shards. |
...
| **project-context-master.md** | File tổng hợp tri thức dự án (chính là tài liệu này), cung cấp bối cảnh tổng quan cho các QC Agent. | Draft (Đang tạo) | Đầu ra của skill `qc-context-master`. |
...
| **System/site/module overview** | Ready | High | Hiểu sai kiến trúc Workers/DO có thể thiết kế kịch bản test concurrency không thực tế | N/A |
| **Feature/use case inventory** | Partial | Medium | Bỏ sót các tính năng cốt lõi khi kiểm thử tích hợp | Rà soát danh sách 14 tính năng Ex Bot được đề xuất. |
...
| **Common rules/data/state/integration** | Ready | High | Bỏ sót quy tắc 30% fee cap hoặc đè nonce | Xác nhận Hyperliquid error code mapping cho các kịch bản lỗi. |
...
**Kết luận:**  
Context cấp project hiện tại đã **Tạm đủ** để các QC Agent hiểu tổng quan hệ thống và bắt đầu thiết kế kịch bản kiểm thử. Thiếu sót lớn nhất và có rủi ro cao nhất là các địa chỉ deploy thực tế và thông số Operator/Multi-sig ví cũng như phương pháp test Stop Loss giả lập.
...
|---|---|---|---|---|---|---|
| Q-001 | Địa chỉ deploy BnzaExVault, Operator address, và danh sách ví Multi-sig signers cụ thể là gì? | Cần thiết để cấu hình môi trường test tích hợp và chạy các script giao dịch. | Không thể chạy test tích hợp hoặc chạy thử nghiệm hệ thống. | High | QC Lead / zen | Open |
