...
| FM-XB-07: Lifecycle State Machine | 18 states: idle→preflight→lp_opening→lp_opened→hedge_pre_open→hedge_post_confirmed→stop_placing→stop_verified→active; runtime: hedge_stopped_cooldown, cooldown, parked, lp_rebalancing, lp_closing, closed, safe_mode, error; PAUSED=status-level only | P0 | 1w |
| FM-XB-08: Close/Redeem Operations | Two systems: (A) user_redeem = LP-first instant redemption (on-chain guarantee, hedge close SLA 5 min); (B) bot_safe_close = hedge-first, USDC parked into uninvested_balances; close_operations idempotency ledger prevents double settlement | P0 | 0.5w |

> **EXBOT Phase 0 Gate (blocks Phase A start)**: Prerequisites per SPEC v5.2.6 §0.3 — (1) zen approves SPEC v5.2.6; (2) BnzaExVault deployed, Base + OP addresses/operator/multiSig finalized (2 sets); (3) HL agent key encryption method confirmed; (4) WL stability gate — 7 consecutive days post-launch with zero SAFE_MODE + no major incident. Before all 4 met: SOTATEK prepares D1 schema + Queue skeleton only — no HL integration.
...
2. BnzaExVault deployed on Base + Optimism — addresses, `authorizedOperator`, `multiSig` (2 sets) confirmed by zen
...
| bot_safe_close = hedge-first | Order: close HL → close LP via BnzaExVault → park USDC in `uninvested_balances`. After done: `lifecycle_state = 'cooldown'` → automatic re-entry every 60 min if conditions met. |
