...
> **RETIRED (2026-06-29):** This use case described the old manual agent key submission + admin approval flow.
> The flow has been replaced: on user on-chain deposit, the system (chain indexer + key-provision worker) automatically
> generates a per-user master key and agent key via AWS KMS, registers the agent key as a delegate with HL `approveAgent`,
> and starts the bot automatically — no manual submission or admin approval required.
>
> Current documentation: `srs/spec.md FR-EXBOT-080`, `srs/flows.md F-03`, `usecases/uc-bot-start.md`.

# UC-EXBOT-agent-key: Submit and Approve HL Agent Key (RETIRED)
