...
| I-05 | MISSING_INFO | Minor | queue_idempotency cleanup | UC step 5 (gap note) | UC step 5 ghi rõ gap: "no scheduled cleanup job exists to purge expired rows — table will accumulate over time." Đây là gap đã được nhận diện trong UC. | Tester cần thiết kế test NFR cho bảng tích lũy (row count growth), nhưng không có acceptance criteria cho rate of growth hay cleanup SLA. | Dev team confirm kế hoạch thêm cleanup job. Nếu không có kế hoạch, ghi nhận là known technical debt với monitoring alert. | ✅ Resolved 2026-07-14 — `expires_at` luôn được set (TTL = 1 phút). EventBridge cron job (mỗi 1 giờ) thực thi `DELETE FROM queue_idempotency WHERE expires_at < now`. FR-EXBOT-011 AC đã được cập nhật trong spec.md và UC step 5. |
...
| I-07 | MISSING_INFO | Minor | F-01 flow diagram thiếu step 11 | SRS flows.md F-01; UC step 11 | F-01 flow diagram trong `srs/flows.md` mô tả: EventBridge Scheduler → Scan Worker → Light-Check Worker → Hedge-Sync. Diagram có chú thích `MDO as "ElastiCache Redis"` (Pool Slot0 Cache) cho step 7, nhưng KHÔNG có bước nào hiển thị việc đọc HL Mark Price Cache (ElastiCache Redis) tại step 11 hay việc enqueue `price-near-stop-audit`. Toàn bộ stop monitoring logic không có trong flow diagram. | Tester dựa vào F-01 để hiểu luồng sẽ bỏ sót stop trigger path. Việc không có diagram cho stop monitoring tăng nguy cơ thiếu test coverage cho I-02 path. | BA bổ sung step 11 (HL Mark Price Cache read) và `price-near-stop-audit` enqueue vào F-01, hoặc tạo riêng một diagram cho stop monitoring sub-flow. | ✅ Resolved 2026-07-14 — F-01 đã được update: thêm HL Mark Price Cache participant, stop monitoring sub-flow (step 11), stale check > 120s với band widening + freeze logic, stop trigger guard với PSAQ enqueue, circuit half_open branch, step 12 overrun check block với atomic safe_mode SET + partial-repair enqueue. |
| I-08 | MISSING_INFO | Minor | US-EXBOT-008 không được update với arc-migration | userstories/us-008.md; SRS FR-EXBOT-040 | US-EXBOT-008 (Circuit Breaker) không có changelog entry 2026-07-04 (arc-migration). Tuy nhiên, nội dung circuit breaker (state transitions, failure counting, probe logic) là platform-agnostic — không phụ thuộc D1/CloudflareWorkers/ElastiCache Redis. Do đó, đây là minor gap về traceability chứ không phải functional conflict. | Tester biết US-008 chưa được xác nhận qua arc-migration checklist. | BA xác nhận US-008 không cần update hoặc thêm changelog note "platform-agnostic, no change required for arc-migration". | ✅ Resolved 2026-07-14 — US-EXBOT-008 content là platform-agnostic: tất cả ACs chỉ tham chiếu bảng `circuit_breakers` và business logic, không có Cloudflare/AWS-specific terms. Không cần thay đổi cho arc-migration; không có changelog entry là có chủ ý. |

### §10.2 Dependencies & Blockers

| Dependency | Loại | Ảnh hưởng | Status |
|---|---|---|---|
| OQ-EXBOT-09 (Pool Slot0 Cache refresh interval) | Open Question → Blocks | Test case cho Pool Slot0 Cache staleness boundary value không thể confirm threshold | Open — Pending zen |
| OQ-EXBOT-11 (lpValueUsd formula confirmation) | Open Question → Blocks | AC-LC-07 sẽ cần update nếu zen đề xuất formula khác | Open — Pending zen |
| OQ-EXBOT-12 (7d APR aggregation formula) | Open Question → Partially Resolved | `funding_rolling_metrics` ERD đã có; tuy nhiên `bnza-market-cron` chưa implement ở v1 → AC-LC-09 vẫn bị block ở v1 | Partially Resolved — Pending OQ-EXBOT-12 close |
| `funding_rolling_metrics` ERD definition | Missing artifact | ✅ ERD đã được bổ sung (bảng thật, schema đầy đủ). Bảng được cập nhật bởi `bnza-market-cron` — chưa implement ở v1 | ✅ Resolved 2026-07-14 |
| US-EXBOT-007 AC-007-1 conflict fix | BA action needed | ✅ AC-007-1 đã được sửa: hedge-sync handler (downstream) là nơi set `lifecycle_state='lp_rebalancing'`, không phải light-check | ✅ Resolved 2026-07-14 |

### §10.3 Audit Summary — Scoring

| Khu vực đánh giá | Điểm tối đa | Phát hiện chính | Điểm đạt | Ghi chú |
|---|---|---|---|---|
| **A1 — Completeness of requirement coverage** | 25 | ~~I-01 (FR Trace thiếu FR-012/040, Minor)~~, ~~I-07 (F-01 diagram thiếu step 11, Minor)~~ — **cả 2 đã resolved 2026-07-14** | 25 | FR trace và flow diagram đã đầy đủ sau BA update |
| **A2 — Internal & cross-source consistency** | 25 | ~~I-02 (US-007 AC-1 conflict, **Major**)~~ — **resolved 2026-07-14** | 25 | Conflict đã được fix; US-007 AC-007-1 nhất quán với UC step 10 và SRS |
| **A3 — Clarity & testability** | 20 | I-04 (staleness threshold ambiguous, Minor — pending OQ-09), I-06 (OQ-11 status mismatch, Minor — pending zen) | 16 | Còn 2 minor ambiguity phụ thuộc OQ chưa closed |
| **A4 — Traceability & evidence** | 15 | ~~I-03 funding_rolling_metrics ERD~~ — **partially resolved 2026-07-14** (ERD có, nhưng `bnza-market-cron` chưa implement ở v1) | 12 | ERD đã có schema; AC-LC-09 còn block ở v1 do worker chưa implement |
| **A5 — Completeness of preconditions & postconditions** | 15 | ~~I-05 (queue_idempotency cleanup gap)~~ — **resolved 2026-07-14**, ~~I-08 (US-008 migration check)~~ — **resolved 2026-07-14** | 15 | Cả 2 issues đã được giải quyết |
| **Tổng** | **100** | | **93** | Tăng từ 77 → 93 sau BA responses 2026-07-14 |

**Verdict: READY (với 1 blocker còn lại ở v1)**

UC-EXBOT-light-check đã đủ điều kiện thiết kế test cases cho tất cả AC. Tất cả Major/Minor issues trong v3 đã được BA resolve ngày 2026-07-14, ngoại trừ:

**Blocker còn lại (v1 scope):**
- **AC-LC-09 (`funding_alert`)** vẫn bị block ở v1 vì `bnza-market-cron` worker chưa implement. Strategy engine dùng fallback `fundingRate × 8760`. OQ-EXBOT-12 chưa closed chính thức. Test case cho `funding_alert` sẽ được thiết kế khi v1.1 implement `bnza-market-cron`.

**Pending OQs (không block test design chính):**
- OQ-EXBOT-09 (staleness threshold) — ảnh hưởng boundary value test case cho Pool Slot0 Cache
- OQ-EXBOT-11 (lpValueUsd formula) — nếu zen thay đổi formula, AC-LC-07 cần update

**Recommendation:** Tiến hành thiết kế test cases đầy đủ cho AC-LC-01 đến AC-LC-18 (ngoại trừ AC-LC-09). AC-LC-18 có thể test với cleanup job đã có.
