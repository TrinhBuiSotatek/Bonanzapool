...
|---|---|---|---|
| `docs/ba/plans/bnza-sotatek-260519-0000/03_modules/exbot/usecases/uc-bot-start.md` | draft 2026-06-12 | UC chính cần audit | — |
| `docs/ba/plans/bnza-sotatek-260519-0000/03_modules/exbot/usecases/index.md` | 2026-06-12 | Index UC + FR trace | FR trace ở đây liệt kê 7 FR (FR-001..004, 020, 030, 031) nhưng UC body chỉ liệt kê 6 (xem I-02) |
| `docs/ba/plans/bnza-sotatek-260519-0000/03_modules/exbot/userstories/us-001.md` | draft 2026-06-12 | US gắn UC (AC duy nhất hỗ trợ) | 4 AC: happy + 3 negative |
| `docs/ba/plans/bnza-sotatek-260519-0000/03_modules/exbot/userstories/index.md` | 2026-06-12 | Index US, mapping FR | — |
| `docs/ba/plans/bnza-sotatek-260519-0000/03_modules/exbot/srs/spec.md` | v5.2.6 / 2026-06-12 | SRS — nguồn truth hành vi bot | Cảnh báo: dùng cùng prefix FR-EXBOT-* nhưng nội dung KHÁC với FRD |
| `docs/ba/plans/bnza-sotatek-260519-0000/03_modules/exbot/srs/states.md` | draft 2026-06-12 | State machine (18 lifecycle states) | Liệt kê đầy đủ enum và transition |
| `docs/ba/plans/bnza-sotatek-260519-0000/03_modules/exbot/srs/flows.md` | draft 2026-06-12 | Sequence flow F-03 cho bot init | F-03 chi tiết hơn UC main flow ở nhiều bước |
...
| `docs/ba/plans/bnza-sotatek-260519-0000/03_modules/exbot/frd.md` | v0.1.0 / 2026-06-12 | FRD — nguồn yêu cầu chức năng | Cùng prefix FR-EXBOT-* nhưng đánh số KHÁC SRS (I-01) |
...
| Khi `hedge_legs.stop_price` populated | UC-EXBOT-light-check (stop monitoring) + UC-EXBOT-bot-safe-close | Light-check sẽ check `markPrice >= stop_price`; stop monitoring chạy ngay cả khi circuit open | Verify `stop_price` BigDecimal, `stop_cloid` khớp HL `placeReduceOnlyStopMarket` response | SRS FR-EXBOT-014 + FR-EXBOT-032 |
...
**Bảng điểm theo `references/scoring-rubric.md`:**
...
| 5 | UC / Spec Documentation Quality Issues | 15 | 7 | ⚠️ Weak | Cap 8/15 áp dụng do CROSS_SOURCE_CONFLICT FR numbering (I-01) + ambiguous wording `0.70` (I-08). Index ≠ body FR trace (I-02-I-03), message verbatim mismatch (I-16), role conflict FRD-vs-UC (I-15). |
| **Tổng** | — | **100** | **52** | — | — |
...
**Khuyến nghị kết luận (1 đoạn):**

UC-EXBOT-bot-start hiện ở trạng thái **Not Ready** (tổng điểm 52/100, dưới ngưỡng 70). UC mô tả tốt happy path 12-step và actor primary, nhưng có 3 Blocker và nhiều Major issue cản đường test design: (i) xung đột đánh số `FR-EXBOT-*` giữa FRD và SRS làm mọi trace mơ hồ; (ii) hơn nửa số nhánh lỗi quan trọng (agent key expired, builder fee, sim fail, hedge IOC reject, HL unreachable, stop placement fail, reconcile mismatch, worker crash recovery) không có trong UC dù SRS đã định nghĩa với E-code và state transition; (iii) phụ thuộc Phase 0 NV-12 (OQ-EXBOT-03) chưa giải quyết — pool address + `wethIndex` per chain — khoá hoàn toàn test dual-chain. Trước khi thiết kế test cases chi tiết, BA nên giải quyết tối thiểu I-01 (đánh số FR), I-04 (bổ sung alt/exception flows), I-08 (làm rõ ý nghĩa `0.70`), và xác nhận trạng thái I-13 (NV-12). Sau khi xử lý các Blocker, UC nên được re-audit; với việc bổ sung alt flow và payload contract, dự kiến điểm có thể đạt 75-80 (Conditionally Ready) ngay cả khi OQ Phase 0 chưa hoàn tất.
