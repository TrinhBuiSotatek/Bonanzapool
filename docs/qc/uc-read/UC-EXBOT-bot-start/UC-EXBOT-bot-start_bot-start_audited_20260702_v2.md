...
**Thay đổi chính so với v1:** Theo `qc-notes-temp.md` (BA, 2026-07-02): (1) I-06 resolved — trạng thái đích khi stop placement fails là `safe_mode` (không phải `error`) per `states.md`; auto-recovery per FR-EXBOT-050. (2) I-03 resolved — HL API fail tại preflight → bot fail ngay với E-EXBOT-008, không retry. Phát hiện thêm: (3) I-11 mới — UC A4 và A7 ghi `error` state nhưng `states.md` không có chuyển đổi `lp_opening → error`; chỉ có `lp_opening → error` và `hedge_pre_open → safe_mode` (không phải `error`). Cần xác nhận với BA.
...
| `srs/states.md` | 2026-06-29 (changelog 2026-06-29) | State diagram | Lifecycle state machine; key_status 3-state updated 2026-06-29 |
...
| `UC-EXBOT-bot-start_bot-start_questions_20260701_v2.md` | 2026-07-02 | Question backlog | Answers for I-02, I-06 applied |
| `docs/BA/qc-notes-temp.md` | 2026-07-02 | BA notes | Source of I-06 resolution + I-02 partial answer |
...
**Nhận xét readiness:** Actor đã đủ rõ để thiết kế test theo role. I-01 (Signing Lambda thiếu trong UC §1 Actors) đã được giải quyết: Signing Lambda là internal infrastructure component, không phải actor nghiệp vụ. Không gây nhầm lẫn; tester đọc flow steps là đủ.
...
| OP-24 | UPDATE D1 `hedge_legs`: `stop_cloid`, `stop_price`; `lifecycle_state='stop_verified'` sang `'active'` | Sau stop confirmed | ExBot Worker | UC §3 step 11, FR-EXBOT-031 |
...
| DO-05 | `hl_agent_keys` | `key_status` | TEXT | Enum: `active` / `superseded` / `revoked` (3-state per states.md changelog 2026-06-29; không còn `approval_status` 4-state) | SRS states.md §Agent Key Status |
...
| DO-09 | `bots` | `lifecycle_state` | TEXT | Fine-grained: 18 states — xem §F.1.3 | SRS states.md, FR-EXBOT-003 |
...
**Lưu ý v2:** `hl_agent_keys` trong `erd.md` có thể vẫn còn các trường cũ (`encrypted_secret`, `secret_iv`, `approval_status`, v.v.) từ trước khi KMS migration (changelog `spec.md` 2026-06-29). Nếu ERD chưa được cập nhật để xóa các trường này, đây là xung đột nguồn (Issue I-12). Tester phải dùng `states.md` và `spec.md` làm nguồn chính thức.
...
| `error` | error | Cần admin xử lý | — | critical bug / LP NFT anomaly; `lp_opening` fail (A4) | SRS states.md |
| `safe_mode` | safe_mode | Mọi mutation bị chặn | `active` (auto-recovery) hoặc `closed` | HL order fails (hedge_pre_open), stop placement fails (stop_placing) | SRS states.md |
...
| OP-09 (Margin check) | One-bot check pass | `marginBalance >= (lpEthAmount × hedgeRatio × hlOraclePrice / leverage) × 2.0`. Dùng `hlOraclePrice`. Nếu HL API không phản hồi tại bước này → fail ngay với E-EXBOT-008 (BA confirmed, I-03 resolved). | HL getMarginSummary | E-EXBOT-002 (margin thiếu); E-EXBOT-008 (HL unreachable tại preflight) |
| OP-10 (Key status check) | Margin check pass | `hl_agent_keys.key_status='active'` cho user này. Enum 3-state: `active`/`superseded`/`revoked` (per states.md changelog 2026-06-29). Nếu không có row active → block. | D1 control_db `hl_agent_keys` | E-EXBOT-017 |
...
| OP-17 (Compute targetShortEth) | lp_opened confirmed | `targetShortEth = lpEthAmount × hedgeRatio`. Bắt buộc dùng BigDecimal. Phase A hedgeRatio = 0.70. | D1 bot config / `hedge_legs.target_ratio` | — |
| OP-18 (openShortIoc) | targetShortEth computed | Signing Lambda nhận request → KMS `kms:Sign` bằng agent key → gửi lệnh IOC đã ký lên HL. | Signing Lambda, KMS, HL | E-EXBOT-008 (HL unreachable), A10 (HL rejection) |
| OP-20 (Reconcile) | IOC order sent | Fetch `clearinghouseState`, verify actual size vs expected. Sai lệch vượt `drift_threshold = lpValueUsd × 3%` → A11 `partial_repair`. Ngưỡng được BA xác nhận 2026-07-02; test boundary bị block đến khi OQ-EXBOT-011 Closed. | HL clearinghouseState | E-EXBOT-011 (internal) |
...
| OP-23 (placeReduceOnlyStop) | stop_trigger_px computed | Signing Lambda ký payload stop order → HL. Khi fail: bot chuyển sang `safe_mode` (KHÔNG phải `error`), per states.md `stop_placing → safe_mode`. Auto-recovery per FR-EXBOT-050. (I-06 resolved 2026-07-02) | Signing Lambda, KMS, HL | E-EXBOT-009 |
...
| 10 | ExBot Worker | Preflight bước 2: HL margin check | `marginBalance >= required × 2.0` → pass | **A2**: margin thiếu → block E-EXBOT-002 với số tiền cụ thể | HL API fail tại preflight → fail ngay E-EXBOT-008 (không retry, không tạo bot record) — I-03 resolved | UC §3, FR-EXBOT-061 |
...
| 15 | ExBot Worker | `BnzaExVault.vaultMint(...)` on-chain | Nhận `VaultMinted` event; `lifecycle_state='lp_opening'` → `'lp_opened'` | **A4**: LP mint fail → `error` state (states.md: `lp_opening → error`), return funds | **A7**: HL unreachable ở bước này — xem Issue I-11 | UC §3 step 5–6, FR-EXBOT-004 |
...
| 17 | ExBot Worker → Signing Lambda | `openShortIoc(targetShortEth, cloid, hl_user_address)` | Signing Lambda gọi KMS Sign → gửi lệnh đã ký lên HL | — | **A7**: HL unreachable → `safe_mode` per states.md (`hedge_pre_open → safe_mode`); **A10**: HL rejection → xem I-11 | UC §3 step 7, SRS F-03 |
| 18 | ExBot Worker | Reconcile: fetch `clearinghouseState` từ HL | Actual size khớp expected; `lifecycle_state='hedge_post_confirmed'` | **A11**: sai lệch > `drift_threshold` (= `lpValueUsd × 3%`) → enqueue `partial_repair`; alert operator. Boundary test bị block đến khi OQ-EXBOT-011 Closed. | — | UC §3 step 8–9, FR-EXBOT-025 |
...
| 20 | ExBot Worker → Signing Lambda | `placeReduceOnlyStopMarket(stopTriggerPx, size, cloid)` | KMS Sign → HL nhận stop order đã ký; stop confirmed | **A8**: stop placement fail → `safe_mode` (KHÔNG phải `error` per states.md `stop_placing → safe_mode`); auto-recovery per FR-EXBOT-050 (retry khi HL responsive + 3 reconciles succeed; nếu irrecoverable → `bot_safe_close`). I-06 resolved. | — | UC §3 step 10–11, FR-EXBOT-031 |
...
| A4 | LP mint on-chain fails | Step 5 (vaultMint) | `lifecycle_state='error'` (states.md `lp_opening → error` confirmed); return funds (cơ chế on-chain revert hay separate action — I-05 deferred) | — (on-chain event / revert) | UC §4 A4, SRS states.md |
...
| A7 | HL unreachable | Step 5 hoặc 7 | Per UC: `lifecycle_state='error'`. **Conflict với states.md:** step 7 failure (`hedge_pre_open`) → `safe_mode` per states.md. (Issue I-11 — xem §10.1) | E-EXBOT-008 | UC §4 A7; SRS states.md |
| A8 | Stop placement fail | Step 10 | `lifecycle_state='safe_mode'` (KHÔNG phải `error`); auto-recovery per FR-EXBOT-050: retry khi HL responsive + 3 reconciles succeed; nếu irrecoverable → `bot_safe_close`. **I-06 RESOLVED** — UC A8 đã được BA xác nhận và fix. | E-EXBOT-009 | UC §4 A8; states.md `stop_placing → safe_mode`; qc-notes-temp.md |
| A10 | HL rejects IOC order | Step 7 | Per UC: `lifecycle_state='error'`. **Conflict với states.md:** step 7 (`hedge_pre_open`) → `safe_mode` không phải `error` khi HL order fails. (Issue I-11) | — (HL response) | UC §4 A10; SRS states.md |
| A11 | Reconcile mismatch | Step 8 | Actual size sai lệch > `drift_threshold` (`lpValueUsd × 3%`, BA confirmed) → enqueue `partial_repair`; alert operator. Boundary test deferred đến OQ-EXBOT-011 Closed. | E-EXBOT-011 (internal) | UC §4 A11 |
...
| LP NFT được giữ bởi BnzaExVault (`positions.custodian='vault'`) | UC-EXBOT-user-redeem, UC-EXBOT-bot-safe-close | Khi close, `vault.executeStrategy(RedeemStrategyV1, ...)` cần tokenId chính xác từ `positions` | `positions.token_id` phải match on-chain | FR-EXBOT-070 |
| Stop placement fail → `safe_mode` (A8) | UC-EXBOT-hedge-sync (blocked), UC-EXBOT-bot-safe-close (trigger khi irrecoverable) | Bot ở `safe_mode` — hedge-sync bị chặn; auto-recovery per FR-EXBOT-050; nếu fail → `bot_safe_close` | `bots.lifecycle_state='safe_mode'`; `bots.status='safe_mode'` | FR-EXBOT-050, states.md |
...
| AC-07 | HL unreachable during preflight margin check | HL API down tại bước 2 preflight (OP-09) | ExBot Worker gọi HL getMarginSummary | Fail ngay với E-EXBOT-008; không tạo bot record (I-03 resolved) | UC §3, FR-EXBOT-002; BA confirmed |
| AC-08 | Stop placement fails → safe_mode | HL nhận short nhưng reject lệnh stop market | ExBot Worker thực thi bước đặt stop (step 10) | `lifecycle_state='safe_mode'` (KHÔNG phải `error`); E-EXBOT-009; auto-recovery per FR-EXBOT-050 kích hoạt | UC §4 A8; states.md `stop_placing → safe_mode`; I-06 resolved |
| AC-09 | HL IOC order rejected | HL reject lệnh short IOC | ExBot Worker gửi short IOC qua Signing Lambda | Hành vi cần xác nhận — UC ghi `error` nhưng states.md chỉ có `hedge_pre_open → safe_mode` (Issue I-11) | UC §4 A10; **Suy luận cần xác nhận** |
| AC-10 | Reconcile mismatch | Actual HL position size sai lệch > `drift_threshold` (`lpValueUsd × 3%`) | Post-hedge reconcile (step 8) | Enqueue `partial_repair`; operator alert. Boundary test bị block đến OQ-EXBOT-011 Closed. | UC §4 A11; I-02 partial answer |
...
| AC-12 | wethIndex per chain | Bot start trên Base và Optimism | Sau LP mint | `positions.weth_index` đúng per chain; không hardcode | FR-EXBOT-004; OQ-EXBOT-03 cần resolve trước |
| AC-13 | BigDecimal cho stop price | Bot start thành công | Sau stop placement | `hedge_legs.stop_price` < `hedge_legs.liquidation_price`; khoảng cách ≥ 30% (stopSafetyFactor=0.70) | FR-EXBOT-030; Suy luận cần xác nhận |
...
| Multi-chain | Base và Optimism đều phải chạy được; `weth_index` verify per chain | Test: bot trên Base và Optimism, weth_index đúng mỗi chain | NFR-EXBOT-009, FR-EXBOT-004; OQ-EXBOT-03 |
| Rate limit | HL API: ≤ 800 weight/min. Bước preflight margin check (weight=2) và reconcile đều consume weight bất kể kết quả downstream. | Test: monitor HL weight consumption trong bot-start flow; nhiều preflight fail liên tiếp đều tốn weight | NFR-EXBOT-004, FR-EXBOT-091; I-09 resolved |
...
|---|---|---|---|---|---|---|---|
| I-02 | High | MISSING_INFO | UC §4 A11 vs FR-EXBOT-025, OQ-EXBOT-011 | UC mô tả A11 là "actual size deviates > threshold". BA đã xác nhận (2026-07-02) formula `drift_threshold = lpValueUsd × 3%` và behavior logic của A11. Tuy nhiên zen chưa confirm OQ-EXBOT-011 — boundary test vẫn bị block cho đến khi OQ-EXBOT-011 Closed. | Boundary test cho reconcile mismatch bị block | zen (OQ-EXBOT-011) | Open (partial) |
| I-04 | High | CROSS_SOURCE_CONFLICT | UC §3 step 6 vs OQ-EXBOT-03 | Pool address + `wethIndex` cho USDC/WETH 0.3% trên Base và Optimism chưa được xác nhận (OQ-EXBOT-03 vẫn Open). Tester cần giá trị cụ thể để verify `positions.weth_index` đúng per chain. | Blocker cho test dual-chain LP open (AC-12) | zen / SOTATEK NV-12 | Deferred |
| I-05 | High | UNCLEAR_INFO | UC §4 A4 | A4 ghi "enter `error` state, return funds" khi LP mint on-chain fails. Cơ chế "return funds" không được mô tả: BnzaExVault tự hoàn tiền qua on-chain revert, hay ExBot Worker cần gọi hàm riêng? Ảnh hưởng postcondition của test case A4. | Block verification postcondition A4 | BA / zen (BnzaExVault owner) | Deferred |
| I-07 | Medium | MISSING_INFO | OQ-EXBOT-05 | Preflight bước 4 kiểm tra "builder fee (5bps) confirmed on HL" nhưng cơ chế check chưa rõ: gọi HL API endpoint nào? Dùng weight bao nhiêu? OQ-EXBOT-05 trong SRS vẫn Open. | Block test case độc lập cho preflight builder fee check | Tech Lead | Deferred |
| I-11 | High | CROSS_SOURCE_CONFLICT | UC §4 A7/A10 vs SRS states.md | UC ghi A7 (HL unreachable) và A10 (HL rejection) → `lifecycle_state='error'`. Nhưng states.md `stop_placing → safe_mode` và `hedge_pre_open → safe_mode` (KHÔNG phải `error`). Chỉ `lp_opening → error` được confirmed trong states.md. Câu hỏi: khi HL order fail tại bước `hedge_pre_open` (A7/A10) — trạng thái đích là `error` hay `safe_mode`? Nếu là `safe_mode` thì recovery flow là gì (như A8 per FR-EXBOT-050)? | Blocker cho test expected state sau A7/A10; test case AC-09 không thể verify expected result | BA | Open |
| I-12 | Medium | CROSS_SOURCE_CONFLICT | `srs/erd.md` vs `srs/spec.md` changelog 2026-06-29 | ERD có thể vẫn còn các trường cũ trong `hl_agent_keys` (`encrypted_secret`, `secret_iv`, `secret_auth_tag`, `wrapped_dek`, `dek_iv`, `encryption_key_version`, `approval_status`, `approved_at`, `approved_by`) từ trước khi KMS migration. Nếu ERD chưa được cập nhật để xóa các trường này → tester thiết kế test case dựa trên ERD sẽ nhầm schema. `states.md` và `spec.md` changelog 2026-06-29 là nguồn chính thức: `key_status` 3-state (`active`/`superseded`/`revoked`), không còn `approval_status`. | Tester cần biết schema thực tế của `hl_agent_keys` để design test case | BA / Tech Lead | Open |

**Issues resolved kể từ v1:** I-01 (Signing Lambda actor — answered), I-03 (HL API fail tại preflight — answered), I-06 (A8 state conflict — answered), I-08 (obsolete E-codes — answered), I-09 (HL weight consumption — answered), I-10 (builder fee check — answered).

**Issues còn lại: 6** (I-02 partial, I-04 deferred, I-05 deferred, I-07 deferred, I-11 new blocker, I-12 new)
**Blocker issues: 2** (I-11, I-02 partial)
...
| OQ-EXBOT-011: Xác nhận `drift_threshold = lpValueUsd × 3%` | Data / Business rule | Block boundary test reconcile mismatch (AC-10) | zen | Open |
| I-11: Xác nhận trạng thái đích của A7/A10 (error hay safe_mode) | UC documentation | Block AC-09 expected result | BA | Open |
...
|---|---|---|---|---|---|
| 1 | Function / Operation & Data Object Inventory | 20 | 17 | ⚠️ Partial | Operation, data objects, state enum, messages được liệt kê đầy đủ và nguyên tử. +1 so với v1 nhờ: I-03 resolved (HL fail tại preflight rõ hành vi), `key_status` 3-state confirmed. Còn trừ: reconcile threshold chỉ partial (OQ-EXBOT-011 chưa closed); wethIndex per chain chưa confirm (OQ-EXBOT-03). |
| 2 | Data Object / State Attributes, Business Rules, Validations & Messages | 25 | 21 | ⚠️ Partial | +3 so với v1: I-06 resolved → A8 state transition rõ (`safe_mode`, auto-recovery per FR-EXBOT-050); I-03 resolved → HL preflight fail behavior rõ. Còn trừ: A7/A10 state conflict (I-11 blocker); ERD `hl_agent_keys` có thể stale (I-12). |
| 3 | Functional Logic & Workflow Decomposition | 25 | 20 | ⚠️ Partial | +2 so với v1: A8 recovery flow đã rõ (safe_mode + auto-recovery); HL preflight fail path rõ (I-03 resolved). Còn trừ: A7/A10 state destination mâu thuẫn UC vs states.md (I-11 blocker); A4 return funds mechanism chưa rõ (I-05 deferred). Cap F.3 không còn áp dụng vì A8 exception path đã có recovery spec. |
| 4 | Functional Integration & Data Consistency | 15 | 12 | ⚠️ Partial | +1 so với v1: safe_mode integration effect được thêm (stop placement fail → safe_mode triggers bot_safe_close nếu irrecoverable). Còn trừ: wethIndex dependency chưa confirm (I-04/OQ-EXBOT-03); BnzaExVault ABI stub (OQ-EXBOT-08). |
| 5 | UC / Spec Documentation Quality Issues | 15 | 11 | ⚠️ Partial | +1 so với v1: I-01 closed, I-08 closed. Trừ điểm cho: I-11 new blocker (A7/A10 vs states.md conflict); I-12 new (ERD stale fields). |
...
So với v1 (73/100), UC-EXBOT-bot-start đã cải thiện đáng kể nhờ 6 issues được giải quyết: A8 state transition đã chính xác (`safe_mode` với auto-recovery per FR-EXBOT-050), HL API fail tại preflight có hành vi rõ ràng, các E-code obsolete đã được xác nhận xóa, và builder fee re-check behavior đã được làm rõ.

**Có thể tiến hành thiết kế test ngay** cho: happy path (AC-01), one-bot policy (AC-02), margin check (AC-03), key not provisioned (AC-04), simulation fail (AC-06), HL unreachable tại preflight (AC-07 — behavior đã rõ), stop placement fail → safe_mode + auto-recovery (AC-08 — I-06 resolved), security/idempotency (AC-11, AC-14).

**Cần resolve trước khi test đầy đủ:**
1. **I-11** (High — New): Xác nhận trạng thái đích của A7/A10 — UC ghi `error` nhưng `states.md` có `hedge_pre_open → safe_mode`. Blocker cho AC-09 expected result.
2. **I-02 / OQ-EXBOT-011** (High): Zen xác nhận `drift_threshold` để unlock boundary test AC-10.
3. **I-12** (Medium): BA/Tech Lead xác nhận ERD `hl_agent_keys` đã được cập nhật bỏ các trường cũ từ KMS migration.
4. **I-04 / OQ-EXBOT-03** (High — Deferred): Confirm pool address + wethIndex per chain để unlock AC-12.
...
|---|---|---|---|
| v1 | 2026-06-30 | QC UC Read ExBot Agent | Tạo báo cáo audited lần đầu — SRS-first cross-check; 10 open issues |
| v2 | 2026-07-02 | QC UC Read ExBot Agent | Cập nhật theo BA answers (qc-notes-temp.md): I-06 resolved (A8 → safe_mode, auto-recovery FR-EXBOT-050); I-03 resolved (HL fail tại preflight → E-EXBOT-008 ngay); I-01/I-08/I-09/I-10 resolved. Phát hiện mới: I-11 (A7/A10 state conflict UC vs states.md), I-12 (ERD stale fields). Điểm: 73 → 81. |
