...
**Điểm quan trọng nhất của kỳ re-audit này (2026-07-27):** kỳ audit trước (v6, 80/100) phát hiện mâu thuẫn nghiêm trọng chưa giải quyết giữa UC và US-EXBOT-002 về isAdmin guard — ExBot Lambda có một điều kiện chặn cứng khiến bất kỳ caller nào không có quyền Admin (tức là Investor, actor chính của UC này) sẽ nhận về lỗi 403 ngay trước khi business logic của endpoint chạy, nhưng US-EXBOT-002 vẫn viết cả 3 Acceptance Criteria cho actor Investor với kỳ vọng response 200, không có ghi chú v1 gap nào (V-009). BA đã xác nhận và cập nhật `us-002.md` ngày 2026-07-24: cả 3 AC (AC-EXBOT-002-1/2/3) giờ có ghi chú rõ "không testable với actor Investor trong v1 — test bằng Admin; full acceptance từ v1.1 khi isAdmin guard được gỡ". Mâu thuẫn UC-US đã được giải quyết dứt điểm — đây là lý do chính khiến điểm tổng của v7 tăng trở lại so với v6.

Cùng lúc, BA đã trả lời và cập nhật nguồn cho 3 vấn đề Minor còn lại của v6: đồng bộ bảng mirror `message-list.md` với E-EXBOT-030/031 (V-010); cập nhật endpoint path trong `us-002.md` §Notes thành `/api/exbot/status/{botId}` (V-011); và xác nhận nội dung response body cụ thể cho lỗi 403 isAdmin guard là `{ "reason": "forbidden" }`, không có E-EXBOT code, đã bổ sung vào UC §2 Preconditions (V-012). Toàn bộ 4 câu hỏi mở của v6 (V-009 đến V-012) nay đã Answered và có nguồn tài liệu cập nhật trực tiếp, không còn phụ thuộc riêng vào lời xác nhận qua question-backlog.

V-001 (`runtime_health_status` vs `health_status`) — vấn đề còn lại từ v5/v6 — vẫn giữ trạng thái như v6: bằng chứng kỹ thuật đã đủ mạnh (changelog `srs/erd.md` ngày 2026-07-20 cho thấy dev đã sửa tên cột ERD thành `runtime_health_status` để khớp UC, xác nhận qua code `runtime-state.ts` và `get-bot-status` handler) nhưng câu trả lời chính thức của BA trong question-backlog vẫn chỉ lặp lại nguyên văn câu hỏi, chưa phải một xác nhận thực sự bằng lời — kỳ audit này không nhận được thông tin mới cho V-001 nên giữ nguyên Open.
...
|---|---|---|---|
| UC-EXBOT-monitor-status | View Active ExBot Status | — (không có version field; cập nhật lần cuối 2026-07-24) | Draft |

| Người viết / BA | Người duyệt | Ngày tạo | Cập nhật lần cuối |
|---|---|---|---|
| @hienduong | — | 2026-06-18 | 2026-07-24 |

| Artefact đã đọc | Version / ngày cập nhật | Vai trò của artefact | Ghi chú |
|---|---|---|---|
| `usecases/uc-monitor-status.md` | 2026-07-24 | UC (primary input) | Cập nhật 2026-07-24: bổ sung nội dung response body cụ thể cho lỗi 403 isAdmin guard vào Preconditions (`{ "reason": "forbidden" }`, không có E-EXBOT code) — đóng V-012 |
| `userstories/us-002.md` | 2026-07-24 | US linked to this UC | Cập nhật 2026-07-24: (1) thêm ghi chú v1 gap vào cả 3 AC (AC-EXBOT-002-1/2/3) — không testable với actor Investor trong v1, test bằng Admin; full acceptance từ v1.1 — đóng V-009; (2) §Notes đã sửa endpoint thành `/api/exbot/status/{botId}` khớp UC — đóng V-011 |
| `srs/spec.md` | (không version field; đã kiểm tra §5 Error Codes) | SRS baseline — nguồn sự thật cho mọi FR và E-code | Xác nhận E-EXBOT-030 và E-EXBOT-031 đã có trong §5, đúng như UC trích dẫn |
...
| `02_backbone/common-rules.md` | (đọc toàn bộ) | Common Rules | **Không có section BR-EXBOT-*** trong file này — BR-EXBOT-007 phải trace về `srs/spec.md` §4, không phải `common-rules.md` |
| `02_backbone/message-list.md` | Changelog 2026-07-24 | E-EXBOT-* mirror registry | Cập nhật 2026-07-24: đã sync E-EXBOT-030 và E-EXBOT-031 vào bảng EXBOT mirror — đóng V-010 |
| `UC-EXBOT-monitor-status_monitor-status_questions_20260715_v4.md` | Cập nhật in-place 2026-07-27 | Question backlog | V-009, V-010, V-011, V-012 → Answered (@hienduong, `qc-responses-2026-07-24.md`, 2026-07-24) |
| `docs/BA/qc-responses-2026-07-24.md` | 2026-07-24 | BA response file | Nguồn trả lời trực tiếp cho V-009, V-010, V-011, V-012 (tất cả Closed/Answered) |
| `UC-EXBOT-monitor-status_monitor-status_audited_20260722_v6.md` | 2026-07-22 | Audit report v6 (80/100, CONDITIONALLY READY) | Baseline so sánh cho v7 |
...
**Nhận xét về mức độ sẵn sàng:** Actor được định nghĩa rõ ràng. Trong v1, chỉ actor Admin thực sự thực thi được toàn bộ luồng; actor Investor — actor chính (Primary) của UC — bị chặn hoàn toàn bởi isAdmin guard. Đây là điểm quan trọng nhất tester cần nắm trước khi thiết kế test case theo role. Mâu thuẫn tài liệu giữa UC và US-EXBOT-002 về hành vi này đã được BA giải quyết ngày 2026-07-24 (V-009 nay Resolved — xem §10.1); US-EXBOT-002 hiện đã có ghi chú v1 gap khớp với UC ở cả 3 AC, nên tester có thể tin tưởng cả hai tài liệu khi thiết kế test case theo actor.
...
| 8 | **v1 gap:** caller phải có quyền Admin — ExBot Lambda có isAdmin guard chặn cứng, non-Admin caller (Investor) nhận 403 với response body `{ "reason": "forbidden" }` (không có E-EXBOT code) trước khi bất kỳ business logic nào chạy; đường xử lý Investor đang chờ triển khai (chi tiết response body xác nhận bởi BA 2026-07-24 — V-012 Resolved) | Có (chặn toàn bộ actor Investor trong v1) | UC §2 |
...
| Investor truy vấn status (v1) | 403 Forbidden với body `{ "reason": "forbidden" }` do isAdmin guard — không đến được business logic, bất kể `wallet_address` có khớp hay không | UC §2 v1 gap |
...
| A9 — wallet mismatch | `wallet_address` của Investor không khớp `bot.user_wallet_address` | 403 E-EXBOT-030 "Access denied: this bot does not belong to your account." | UC §4 A9 | **v1 gap:** không thể thực thi bằng actor Investor trong v1 vì đã bị isAdmin guard chặn trước khi tới bước kiểm tra ownership; hành vi này đã được US-EXBOT-002 xác nhận khớp (V-009 Resolved) |
| A10 — paused | `bots.status='paused'` (status overlay — `lifecycle_state` giữ nguyên giá trị trước khi pause, thường là `active`) | 200 với `status='paused'`, `lifecycle_state=<giá trị được giữ nguyên>`; UI hiển thị E-EXBOT-031 "Bot Paused. Hedge and LP are maintained. You may still redeem." | Tất cả nút mutation bị tắt; riêng `user_redeem` vẫn được phép theo State Registry | **v1 gap:** chưa thể kiểm thử đầy đủ cho actor Investor cho đến khi isAdmin guard được gỡ bỏ (V-008 Answered; US-EXBOT-002 đã có ghi chú v1 gap khớp — V-009 Resolved) |
...
| Caller không có quyền Admin (v1 gap isAdmin guard) | Lỗi API | 403 Forbidden — chặn trước business logic; response body `{ "reason": "forbidden" }`, không có message code E-EXBOT riêng (BA xác nhận 2026-07-24 — V-012 Resolved) | Không có E-EXBOT code | UC §2 (changelog 2026-07-20, 2026-07-24) |
...
| Bot ở trạng thái `paused` | Response API (200) | "Bot Paused. Hedge and LP are maintained. You may still redeem." | E-EXBOT-031 | UC §4 A10, `srs/spec.md` §5; đã sync vào `message-list.md` mirror ngày 2026-07-24 (V-010 Resolved) |
...
| isAdmin guard (nội bộ ExBot Lambda) | In-process check | Không đọc/ghi dữ liệu; chỉ kiểm tra role của caller | Guard chạy trước bất kỳ Aurora PG/Cache read nào | **Rủi ro cao (vẫn còn hiệu lực trong v1):** mọi test scenario theo actor Investor (bao gồm happy path, A1–A10 phía Investor) đều bị chặn ở tầng này trong v1 — không phản ánh đúng luồng nghiệp vụ dự kiến cuối cùng, chỉ có thể verify hành vi 403 với body `{ "reason": "forbidden" }`; test coverage đầy đủ cho Investor phải chờ v1.1. Mâu thuẫn tài liệu trước đây giữa UC và US-EXBOT-002 về hành vi này đã được giải quyết (V-009 Resolved) — rủi ro còn lại chỉ là rủi ro nghiệp vụ (thiếu coverage Investor), không còn là rủi ro tài liệu | UC §2 (changelog 2026-07-20, 2026-07-24) |
...
|---|---|---|---|---|---|---|
| AC-ms-01 | US-EXBOT-002 AC-EXBOT-002-1 | Bot ở trạng thái `active`; `lp_eth_amount > 0`; Pool Slot0 Cache có `currentTick`; `currentTick` nằm trong `[tickLower, tickUpper)`; caller có quyền Admin | Admin gọi `GET /api/exbot/status/{botId}` | 200 OK; `lifecycle_state='active'`; `range_state='in'`; `drift_pct` là số thập phân dương; các field v1 gap (`last_reconcile_at`, `last_error_code`, `safe_mode_tier`, `safe_mode_reason`, `cooldown_end_at`) = `null` | Happy path | Happy path chỉ thực thi được với actor Admin trong v1; US-EXBOT-002 AC-EXBOT-002-1 hiện đã có ghi chú v1 gap khớp với hành vi này (V-009 Resolved) |
| AC-ms-01b | Suy luận cần xác nhận | Bot ở trạng thái `active`; caller là Investor có `wallet_address` khớp `bot.user_wallet_address` | Investor gọi `GET /api/exbot/status/{botId}` | 403 Forbidden với body `{ "reason": "forbidden" }` do isAdmin guard — **không phải** 200 như mô tả gốc ban đầu của US-EXBOT-002 AC-EXBOT-002-1 | Alternate — v1 gap (đã xác nhận, không còn là gap tài liệu) | Mới thêm ở v6 để ghi nhận gap giữa UC và US; US-EXBOT-002 đã cập nhật ghi chú v1 gap khớp (V-009 Resolved) — AC này vẫn giữ để test coverage rõ ràng cho response 403 |
...
| AC-ms-05 | Suy luận cần xác nhận | Admin gọi API thay mặt Investor với `wallet_address` không khớp bot (theo lý thuyết) | Admin/Investor gọi `GET /api/exbot/status/{botId}` | Về lý thuyết: 403 E-EXBOT-030; **thực tế trong v1:** nếu caller là Investor, request đã bị 403 isAdmin guard (body `{ "reason": "forbidden" }`) chặn trước khi tới bước kiểm tra ownership — không thể phân biệt hai loại 403 bằng test case ở tầng API trong v1 | Error A9 (Blocked bởi v1 gap) | Thứ tự guard (isAdmin trước wallet-match) đã được xác nhận; mâu thuẫn tài liệu liên quan đã đóng (V-009 Resolved) |
...
| AC-ms-08 | US §4 A10 | Bot ở trạng thái `paused` (`bots.status='paused'`); caller có quyền Admin | Admin gọi `GET /api/exbot/status/{botId}` | 200 OK; `status='paused'`; `lifecycle_state` giữ giá trị trước pause; message E-EXBOT-031 | Alternate A10 | **Đã đóng V-008** (v5); `E-EXBOT-031` đã được sync vào `message-list.md` mirror (V-010 Resolved); test theo Investor vẫn bị Blocked bởi isAdmin guard trong v1 |
...
|---|---|---|---|---|---|---|---|
| CH-01 | question-backlog V-009 (Answered, @hienduong, 2026-07-24) + `userstories/us-002.md` update 2026-07-24 | §2 Actor, §6.A (A9), §7 Integration Analysis, §8 (AC-ms-01/01b/05/08), §10.1 (V-009) | Modified | US-EXBOT-002 viết cả 3 AC (AC-EXBOT-002-1/2/3) cho actor Investor, kỳ vọng 200, không có ghi chú v1 gap — mâu thuẫn trực tiếp với UC (Investor nhận 403 do isAdmin guard) | US-EXBOT-002 đã cập nhật cả 3 AC với ghi chú v1 gap khớp UC: "không testable với actor Investor trong v1 — test bằng Admin; full acceptance từ v1.1" | Yes | Yes |
| CH-02 | question-backlog V-010 (Answered, @hienduong, 2026-07-24) + `message-list.md` Changelog 2026-07-24 | §6.C Messages, §8 (AC-ms-08), §10.1 (V-010) | Modified | Bảng mirror EXBOT trong `message-list.md` dừng ở E-EXBOT-029, chưa có E-EXBOT-030/031 | `message-list.md` đã sync đầy đủ E-EXBOT-030 và E-EXBOT-031 vào bảng mirror EXBOT | No | Yes |
| CH-03 | question-backlog V-011 (Answered, @hienduong, 2026-07-24) + `userstories/us-002.md` update 2026-07-24 | §0 Artefact metadata, §1.2 Auth Investor path | Modified | `us-002.md` §Notes ghi endpoint `/api/exbot/status` (không có `{botId}`), không khớp UC | `us-002.md` §Notes đã sửa thành `/api/exbot/status/{botId}` khớp UC | No | No |
| CH-04 | question-backlog V-012 (Answered, @hienduong, 2026-07-24) + `uc-monitor-status.md` update 2026-07-24 | §3.1 Preconditions, §6.C Messages, §10.1 (V-012) | Modified | UC §2 Preconditions chỉ nói "receive 403", không có response body cụ thể cho lỗi isAdmin guard | UC §2 Preconditions đã bổ sung response body cụ thể: `{ "reason": "forbidden" }`, không có E-EXBOT code | No | Yes |

### AC Change Summary

| AC ID | Change type | Old AC content | New AC content | Reason / source |
|---|---|---|---|---|
| AC-ms-01 | Modified | Ghi chú: happy path chỉ Admin verify được, không phải Investor như US-EXBOT-002 mô tả (gap chưa xác nhận) | Ghi chú cập nhật: US-EXBOT-002 AC-EXBOT-002-1 hiện đã có ghi chú v1 gap khớp hành vi này | CH-01 / V-009 Resolved |
| AC-ms-01b | Modified | Kỳ vọng 403 "không phải" 200 như US-EXBOT-002 mô tả — được ghi là gap giữa US và v1 (v1) | Response body cụ thể hoá `{ "reason": "forbidden" }`; ghi chú "gap đã xác nhận, không còn là gap tài liệu"; giữ AC để bảo đảm test coverage cho response 403 | CH-01, CH-04 / V-009, V-012 Resolved |
| AC-ms-05 | Modified | Ghi chú: đã xác nhận thứ tự guard, "xem V-009" | Ghi chú cập nhật: mâu thuẫn tài liệu liên quan đã đóng (V-009 Resolved); bổ sung response body `{ "reason": "forbidden" }` vào Then | CH-01, CH-04 / V-009, V-012 Resolved |
| AC-ms-08 | Modified | Ghi chú: `E-EXBOT-031` chưa có trong `message-list.md` mirror (V-010 Open) | Ghi chú cập nhật: `E-EXBOT-031` đã sync vào mirror (V-010 Resolved) | CH-02 / V-010 Resolved |

### Re-audit Scoring Summary

| Scoring area | Previous score (v6) | New score (v7) | Reason |
|---|---:|---:|---|
| 1. Function/Operation & Data Object Inventory | 18/20 | 20/20 | CH-02, CH-04 đóng các khoản trừ điểm còn lại (message-list.md mirror, response body isAdmin guard) |
| 2. Data Object/State Attributes, BR, Validations & Messages | 23/25 | 23/25 | Không có change nào trong kỳ này ảnh hưởng Area 2; V-001 vẫn trừ điểm nhẹ như v6 |
| 3. Functional Logic & Workflow Decomposition | 17/25 (capped 70%) | 24/25 | CH-01 giải quyết V-009 → gỡ auto-cap 70% theo scoring-rubric §8; trừ điểm nhẹ còn lại vì actor Investor vẫn chỉ verify được response 403, không phải toàn luồng |
| 4. Functional Integration & Data Consistency | 14/15 | 14/15 | Không có change nào trong kỳ này ảnh hưởng Area 4 |
| 5. UC/Spec Documentation Quality Issues | 8/15 (capped) | 13/15 | CH-01 giải quyết V-009 → gỡ auto-cap theo scoring-rubric §8; trừ điểm nhẹ còn lại vì V-001 (Major) vẫn Open |

| Previous total | New total | Previous verdict | New verdict |
|---:|---:|---|---|
| 80/100 | 94/100 | CONDITIONALLY READY | READY |
...
| D2 | Gỡ bỏ isAdmin guard trong ExBot Lambda để mở đường xử lý Investor | Backend / Architecture | Toàn bộ scenario Investor (happy path, A1–A10 dưới góc nhìn Investor) không thể test đầy đủ đến khi gỡ guard này (đây là rủi ro nghiệp vụ còn tồn đọng, không còn là mâu thuẫn tài liệu — V-009 đã Resolved) | Chờ v1.1 |
...
| D4 | Đồng bộ `message-list.md` mirror với `spec.md` §5 (thêm E-EXBOT-030/031, dọn dòng lạc "Symbol not found") | BA / Tech Lead | Không block, chỉ gây bất tiện tra cứu | **Resolved** (2026-07-24 — V-010) |
| D5 | Cập nhật `us-002.md` để phản ánh v1 gap isAdmin guard trong AC | BA / US update | Nếu không cập nhật, tester dựa vào US một cách máy móc sẽ thiết kế test case luôn fail trong v1 | **Resolved** (2026-07-24 — V-009) |
...
|---|---|---|---|
| 1. Function/Operation & Data Object Inventory | 20 | 20 | **Tăng so với v6 (18→20):** message-list.md mirror đã sync đủ E-EXBOT-030/031 (V-010 Resolved) và response body của lỗi isAdmin guard đã được định nghĩa rõ (V-012 Resolved) — không còn khoản trừ nào cho Area này |
| 2. Data Object/State Attributes, BR, Validations & Messages | 25 | 23 | Không đổi so với v6: 7/8 vấn đề Major/Minor của v5 (V-002 đến V-008) đã được BA trả lời đầy đủ; V-001 còn thiếu xác nhận chính thức bằng lời (trừ điểm nhẹ, không đổi trong kỳ này) |
| 3. Functional Logic & Workflow Decomposition | 25 | 24 | **Tăng so với v6 (17→24):** auto-cap 70% theo scoring-rubric §8 đã được gỡ vì mâu thuẫn UC/US-EXBOT-002 về hành vi isAdmin guard đã giải quyết dứt điểm (V-009 Resolved 2026-07-24). Trừ điểm nhẹ còn lại vì actor Investor trong v1 vẫn chỉ verify được qua response 403 (không phải toàn bộ luồng nghiệp vụ) |
| 4. Functional Integration & Data Consistency | 15 | 14 | Không đổi so với v6: Aurora PG 4-bảng, Pool Slot0 Cache null-path, HMAC hai lớp đều rõ ràng; isAdmin guard được mô tả đúng vị trí trong luồng xử lý |
| 5. UC/Spec Documentation Quality Issues | 15 | 13 | **Tăng so với v6 (8→13):** auto-cap đã được gỡ cùng lý do Area 3. Trừ điểm nhẹ còn lại vì V-001 vẫn là issue Major đang Open (dù bằng chứng kỹ thuật đã đủ mạnh, chưa có xác nhận chính thức bằng lời từ BA) |
| **Tổng** | **100** | **94** | |

#### Verdict: READY

**Ghi chú còn lại (không chặn Ready, chỉ khuyến nghị):**

1. **[Recommended — Major với bằng chứng kỹ thuật đã đủ]** BA xác nhận lại chính thức tên field/cột cho `runtime_health_status` (V-001) — bằng chứng ERD hiện tại đã đủ mạnh để tạm coi là resolved về mặt kỹ thuật, nhưng hồ sơ audit cần một câu trả lời thực sự thay vì lặp lại câu hỏi.
2. **[Note — không block]** Rủi ro nghiệp vụ (không phải rủi ro tài liệu) vẫn còn: actor Investor — actor Primary của UC — vẫn bị chặn hoàn toàn bởi isAdmin guard trong v1; test coverage đầy đủ cho Investor (happy path, A1–A10 dưới góc nhìn Investor) phải chờ v1.1 gỡ guard này (D2). Test case theo actor Investor cần đánh dấu "Blocked — chờ v1.1"; test case theo actor Admin có thể thực thi ngay.

**Nhận xét tổng thể:** v7 tăng điểm rõ rệt so với v6 (80/100 → 94/100) vì cả 4 issue mở của v6 (V-009 Major, V-010/V-011/V-012 Minor) đã được BA trả lời đầy đủ ngày 2026-07-24 VÀ được xác nhận có cập nhật trực tiếp vào nguồn (`us-002.md`, `message-list.md`, `uc-monitor-status.md`) — đủ điều kiện đánh dấu Resolved (không chỉ Answered chờ cập nhật nguồn) theo tiêu chí của quy trình re-audit. Đáng chú ý nhất: mâu thuẫn UC-US về isAdmin guard (V-009) — vấn đề khiến v6 bị auto-cap ở cả Area 3 và Area 5 — đã được giải quyết dứt điểm, nên cả hai vùng điểm này được gỡ cap và tăng điểm đáng kể. Lưu ý quan trọng: **bản thân hành vi isAdmin guard chặn Investor trong v1 KHÔNG thay đổi** — đây vẫn là một rủi ro nghiệp vụ thật (actor Primary của UC không thể test đầy đủ đến v1.1) — chỉ có mâu thuẫn *tài liệu* giữa UC và US về hành vi này là đã được giải quyết. V-001 là issue Major duy nhất còn Open, nhưng không đủ điều kiện áp dụng auto-cap Area 5 vì đây là một khoảng trống xác nhận hình thức (bằng chứng kỹ thuật đã đủ mạnh, ERD đã tự sửa khớp UC), không phải một mâu thuẫn làm thay đổi hành vi mong đợi.
...
| v7 | 2026-07-27 | Re-audit sau BA answers `qc-responses-2026-07-24.md` (đóng V-009, V-010, V-011, V-012 — tất cả có cập nhật trực tiếp vào nguồn: `us-002.md`, `message-list.md`, `uc-monitor-status.md`). Mâu thuẫn UC-US về isAdmin guard (V-009) đã giải quyết dứt điểm → gỡ auto-cap ở Area 3 và Area 5 theo scoring-rubric §8. Changed sections: Feature Brief, §0, §1.2 (Auth Investor path note), §2 Actor, §3.1 Preconditions, §3.2 Postconditions, §6.A Alternate flows (A9, A10), §6.C Messages, §7 Integration Analysis, §8 AC (AC-ms-01, 01b, 05, 08), §10.1 Issue Register, §10.2 Dependencies, §10.3 Audit Summary. AC changes: modified 4 (AC-ms-01, AC-ms-01b, AC-ms-05, AC-ms-08), added 0, deleted 0. Điểm 94/100 READY (tăng từ 80/100) | QC UC Read Agent |
