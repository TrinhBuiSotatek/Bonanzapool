...
|---|---|---|---|
| `usecases/uc-pause-resume.md` | 2026-07-03 | UC chính được review | Linked: US-EXBOT-003. Changelog: Q2/Q3: worker architecture + idempotency; Q8: UI boundary; Q9: MSG-SUC-81/82 |
...
| `message-list.md` | 2026-07-03 | E-EXBOT-* + MSG-EXBOT codes | Changelog: MSG-SUC-81/82 added for pause/resume |
...
| ExBot Worker | System | Xử lý pause/resume request như synchronous HTTP POST handler, cập nhật D1, enqueue light-check | Không qua queue — direct synchronous handler | UC §1 (updated 2026-07-03) |
...
✅ **Đã được giải quyết (Q2 — was Blocker):** "Status Update Worker" không tồn tại trong SRS queue topology. BA đã xác nhận: ExBot Worker xử lý pause/resume trực tiếp như synchronous HTTP POST handler (`/pause`, `/resume`) — không có queue riêng. Tên "Status Update Worker" được chuẩn hóa thành "ExBot Worker" trong UC.
...
| 2 | ExBot Worker | Query D1: kiểm tra `bots.status='active'` và `lifecycle_state='active'` | Xác nhận điều kiện thỏa mãn | Nếu `status='safe_mode'` → A1 | — | UC §3 step 2 |
| 3 | ExBot Worker | Set `bots.status='paused'` với conditional UPDATE (`WHERE status='active'`) | Ghi thành công hoặc race condition prevented | — | — | UC §3 step 3 (Q3 fix) |
...
| 2 | ExBot Worker | Query D1: kiểm tra `bots.status='paused'` | Xác nhận điều kiện thỏa mãn | Nếu `status='active'` → A2 | — | UC §4 step 2 |
| 3 | ExBot Worker | Khôi phục `bots.status='active'` với conditional UPDATE (`WHERE status='paused'`) | Ghi thành công hoặc race condition prevented | — | — | UC §4 step 3 (Q3 fix) |
| 4 | ExBot Worker | Đọc `bots.lifecycle_state` từ D1 (không thay đổi), xác nhận giá trị hợp lệ | — | Nếu giá trị bất thường? Không nêu | UC §4 step 4 |
| 5 | ExBot Worker | Set `bot_runtime_state.next_light_check_at = now + 5min + jitter(−45s, +45s)`, enqueue `light-check` message | Message được queue | — | — | UC §4 step 5; FR-EXBOT-013 |
...
| Q2 | Blocker | Missing | FR-EXBOT-010; UC §1, §3 | **"Status Update Worker" không tồn tại trong queue topology.** | ✅ **Đã được giải quyết (2026-07-03):** "Status Update Worker" không phải là một worker riêng biệt. Pause/resume được xử lý trực tiếp bởi ExBot Worker dưới dạng synchronous HTTP POST handler (`/pause`, `/resume`). Không có queue consumer nào liên quan. Tên "Status Update Worker" trong UC được chuẩn hóa thành "ExBot Worker". (Source: `uc-pause-resume.md` changelog 2026-07-03) | — | BA (@hienduong) | **Answered** |
...
| Q2, Q3, Q7, Q8, Q9 | BA responses (2026-07-03) | Đã được giải quyết — ExBot Worker synchronous handler, DB conditional UPDATE idempotency, pause only from `lifecycle_state='active'`, ExBot scope ends at API response, MSG-SUC-81/82 registered | — | **Answered** |
...
| **Tổng điểm** | **100** | **100** | **✅ Ready** | **0 (Q1 deferred)** |

**Chi tiết Area (sau khi Q2, Q3, Q7, Q8, Q9 được BA giải quyết):**

- **Area 1 (20/20):** Q2 (Status Update Worker) đã được giải quyết — ExBot Worker xử lý synchronous. Q7 (lifecycle_state) đã được BA chốt. Điểm tối đa.
...
- **Area 5 (15/15):** Q8 ("UI confirmation" scope) đã được giải quyết — ExBot scope ends at API response. Q9 (message codes) đã được giải quyết. Điểm tối đa.
...
|---|---|---|---|
| Q2 | Blocker | ExBot Worker xử lý pause/resume trực tiếp như synchronous HTTP POST handler (`/pause`, `/resume`) — không có queue riêng. "Status Update Worker" được chuẩn hóa thành "ExBot Worker". | `uc-pause-resume.md` changelog 2026-07-03 |
...
**UC-EXBOT-pause-resume đã đạt trạng thái Ready cho test design.**

- Tất cả blockers và major issues đã được giải quyết bởi BA (2026-07-03) hoặc verified từ docs evidence.
- Chỉ có Q1 (API endpoint names) là deferred — không ảnh hưởng đến test design vì QA sẽ lấy từ environment config.
- Tester có thể tiến hành thiết kế test case ngay lập tức.
