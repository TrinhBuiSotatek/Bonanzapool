...
| I-10 | Low | UC §3 step 5 note; UC §4 A6 | UC §3 step 5 có note "behavior pending OQ-EXBOT-013" nhưng UC §4 A6 đã định nghĩa hành vi delta=0 rõ ràng: "skip HL order entirely; proceed directly to stop replacement (step 8). Log reason in rebalance_attempts." Hai phần trong cùng UC mâu thuẫn: step 5 note nói "pending" trong khi A6 đã có behavior. BA cần xóa note "pending OQ-EXBOT-013" ở step 5 hoặc cập nhật để nhất quán với A6, đồng thời chốt `rebalance_attempts.status` value khi delta=0 để A6 hoàn chỉnh. | Tester không biết nên follow step 5 note hay A6 khi viết test case cho delta=0 scenario | Open |
| I-11 | Low | UC §3 step 8, step 13; SRS FR-EXBOT-030, FR-EXBOT-035 | UC §3 step 8 mô tả INV-STOP: cancel stop → place new stop. Stop mới được đặt ở step 8, nhưng `stop_trigger_px` chỉ được recompute ở step 13 (post-reconcile, dùng `entry_price` mới). Không rõ: (a) stop được đặt ở step 8 dùng giá nào — giá cũ từ `hedge_legs.stop_price` như placeholder rồi replace lại sau reconcile? Hay (b) reconcile cho `entry_price` trước, rồi recompute, rồi mới place stop? BA cần làm rõ sequence giữa INV-STOP (step 8) và stop_trigger_px recompute (step 13). | Tester không thể xác định expected `stop_price` value ở step 8 vs step 13 — hai test assertion khác nhau | Open |
