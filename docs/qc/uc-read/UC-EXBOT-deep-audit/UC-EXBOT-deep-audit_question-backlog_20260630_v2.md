...
|----|----------|-----|----------|----------------|--------|
| Q-DA-03 | Medium | UC Step 1; NFR-EXBOT-012 | **Shard iteration strategy cho AWS Lambda deep-audit worker (architectural component):** UC Step 1 nói Lambda function đọc bots từ Aurora PostgreSQL nhưng không specify Lambda đọc tất cả shards một lần rồi xử lý tuần tự, hay có một EventBridge Scheduler trigger per shard để iterate qua shards. Đây là Tech Lead decision về Lambda architecture. Behavior nghiệp vụ không thay đổi dù theo cách nào. | Multi-shard test design cho Phase B (4 shards) phụ thuộc vào Lambda worker shard iteration strategy. Phase A test không bị ảnh hưởng. | Open |
| Q-DA-04 | Low | UC A1; erd.md | **Cadence switching mechanism cho AWS Lambda deep-audit worker (architectural component):** UC A1 nói "update cadence to high-risk interval (1 hour)" khi HL unreachable nhưng không rõ Lambda worker có update `bots.next_deep_audit_at` field trong Aurora PostgreSQL không, hay EventBridge Scheduler tự switch schedule dựa trên state query mà không cần Lambda ghi. Đây là Tech Lead decision về Lambda-EventBridge integration. | Tester cần verify cadence switching mechanism để design test đúng behavior. Đây là Tech Lead decision về Lambda-EventBridge integration. | Open |
