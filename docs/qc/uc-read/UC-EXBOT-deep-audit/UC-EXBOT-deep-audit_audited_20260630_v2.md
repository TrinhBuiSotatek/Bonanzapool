...
**Version:** v2 (re-audit after BA document update 2026-07-04)
...
| HL Rate Limiter (Redis) + User Lock (Redis Redlock) interaction | OQ-EXBOT-15: xác định weight consumption ordering | Tech Lead decision |
...
|---|---|---|
| Mismatch detected (Step 3) | Aurora PostgreSQL record mismatch; bot enters SAFE_MODE; admin notification queued | UC A3 |
...
| 2 | Deep-Audit Worker | Gọi HL `clearinghouseState` | Worker calls HL via HL Rate Limiter (ElastiCache Redis) (weight=2); fetches actual short size | **A1 (HL unreachable):** Rate Limiter returns `{allowed: false}` hoặc HL returns 5xx → skip Steps 2,3,4,6,7; update cadence to 1h; enqueue notification | — | UC Step 2; FR-EXBOT-091 |
| 3 | Deep-Audit Worker | Verify hedge size match | Worker compares actual short size from HL với `hedge_legs.last_known_hl_short_size` | **A3 (mismatch):** actual ≠ last_known → record mismatch in Aurora PostgreSQL; trigger SAFE_MODE; enqueue admin notification với size delta | — | UC Step 3 |
...
|---|---|---|---|---|---|
| HL weight budget | Deep-audit calls HL với weight=2; BR-EXBOT-003 (HL weight = 0) không apply cho deep-audit | Yes | Gọi HL thành công | A1: HL Rate Limiter returns `{allowed: false}` → skip HL steps, switch to 1h cadence | UC BR; FR-EXBOT-091 |
...
| Cadence switching logic | EventBridge Scheduler cadence chuyển 6h → 1h khi ANY of: `circuit_breakers.state != 'closed'`, `margin_status='warning'`, `margin_status='critical'` | Yes | Worker chạy ở high-risk schedule | N/A | UC §2 Preconditions; FR-EXBOT-016 |
...
| HL Rate Limiter (ElastiCache Redis) interaction | FR-EXBOT-091: HL Rate Limiter đệm tất cả outbound HL API calls; OQ-EXBOT-15 (Tech Lead) xác định weight consumption ordering | Yes (Tech Lead decision pending) | HL calls được rate-limit đúng | Rate limit exceeded → A1 path | FR-EXBOT-091; OQ-EXBOT-15 |
...
| HL Rate Limiter (ElastiCache Redis) | FR-EXBOT-091; OQ-EXBOT-15 | Deep-audit gọi HL qua rate limiter với weight=2; Tech Lead xác định consumption ordering | Tester cần verify rate limiter được gọi đúng trước/sau HL call | FR-EXBOT-091; OQ-EXBOT-15 |
...
| AC-DA-02 | Mismatch detection → SAFE_MODE | Bot active; actual short size trên HL ≠ `hedge_legs.last_known_hl_short_size` | Deep-audit worker verifies size | Bot enters `safe_mode`; Aurora PostgreSQL records mismatch; admin notification với size delta enqueued | UC A3; FR-EXBOT-050; E-EXBOT-011 |
...
| AC-DA-05 | HL unreachable → graceful degradation | HL Rate Limiter returns `{allowed: false}` hoặc HL API returns 5xx | Deep-audit worker attempts HL call | Steps 2,3,4,6,7 skipped; cadence chuyển 1h; notification "Hyperliquid API unreachable" (E-EXBOT-008) enqueued; retry-pending state recorded | UC A1; E-EXBOT-008 |
...
| AC-DA-07 | High-risk cadence switching | `circuit_breakers.state='open'` OR `margin_status='warning'` OR `margin_status='critical'` | EventBridge Scheduler evaluates conditions | Deep-audit schedule switches from 6h to 1h | UC §2 Preconditions; FR-EXBOT-016 |
...
| AC-DA-09 | HL Rate Limiter (ElastiCache Redis) integrated | Deep-audit attempts HL call | Worker calls via HL Rate Limiter (ElastiCache Redis) với weight=2 | Rate limit enforced; `{allowed: false}` → A1 path | FR-EXBOT-091; OQ-EXBOT-15 |
...
| Rate Limit | Deep-audit sử dụng HL API weight = 2. BR-EXBOT-003 (HL weight=0) không apply — deep-audit có weight=2. | Tester cần verify HL Rate Limiter (ElastiCache Redis) được gọi đúng với weight=2 | FR-EXBOT-091; BR-EXBOT-003 exception in UC BR |
...
| Q-DA-03 | Medium | MISSING_INFO | UC Step 1; NFR-EXBOT-012 | Không rõ deep-audit shard iteration strategy cho Phase B (4 shards). Behavior nghiệp vụ không thay đổi dù theo cách nào. Phase A test không bị ảnh hưởng. | Multi-shard test design cho Phase B (4 shards) phụ thuộc vào shard iteration strategy | Tech Lead | Open |
| Q-DA-04 | Low | UNCLEAR_INFO | UC A1; erd.md | Cadence switching mechanism (6h→1h) được ghi vào đâu trong Aurora PostgreSQL chưa rõ. Tech Lead decision về implementation. | Tester cần verify cadence switching mechanism để design test đúng behavior | Tech Lead | Open |
| Q-DA-06 | Medium | MISSING_INFO | UC Step 2; FR-EXBOT-091; OQ-EXBOT-15 | **MỚI (2026-07-06 Re-audit):** Deep-audit Step 2 gọi HL qua HL Rate Limiter (ElastiCache Redis) với weight=2. Tuy nhiên, OQ-EXBOT-15 (vẫn Open trong spec.md §9) xác định Tech Lead cần xác định: HL Rate Limiter weight consumption nên được thực hiện **trước** hay **sau** khi acquire Redis Redlock? Điều này ảnh hưởng đến retry behavior khi rate limit hit trong khi lock đang held. Deep-audit không sử dụng Redis Redlock (không có hedge mutation), nên câu hỏi này ít ảnh hưởng trực tiếp đến deep-audit. Tuy nhiên, FR-EXBOT-091 được thiết kế chung cho cả hedge-sync và deep-audit. | Tech Lead decision về rate limiter ordering ảnh hưởng đến system design tổng thể | Tech Lead | Open |
| Q-DA-07 | Low | MISSING_INFO | UC Step 3; spec.md FR-EXBOT-016 | **MỚI (2026-07-06 Re-audit):** UC Step 3 verify actual short size từ HL matches `last_known_hl_short_size`. Khi mismatch được phát hiện (→ SAFE_MODE), UC không mô tả rõ liệu `last_known_hl_short_size` có được update thành actual size từ HL không, hay để nguyên giá trị mismatch. Điều này ảnh hưởng đến behavior sau khi bot recover từ SAFE_MODE. | Tester cần biết state của `last_known_hl_short_size` sau mismatch detection để design recovery test đúng | Tech Lead / BA | Open |
...
| HL Rate Limiter (ElastiCache Redis) (FR-EXBOT-091) | Infrastructure | Deep-audit sử dụng HL Rate Limiter. Weight budget phải được config đúng. OQ-EXBOT-15 Open. | Tech Lead | Draft |
...
| OQ-EXBOT-15 (HL Rate Limiter ordering) | Open Question | Weight consumption ordering — Tech Lead decision | Tech Lead | Open |
...
## 11. Issue Register (Updated 2026-07-06 - Re-audit)
...
| I-DA-03 | MISSING_INFO | Medium | Area 1: Function/Operation Inventory | Không rõ deep-audit shard iteration strategy cho Phase B (4 shards). BA xác nhận đây là Tech Lead decision. Phase A test không bị ảnh hưởng. | Open |
| I-DA-04 | UNCLEAR_INFO | Low | Area 2: Business Rules | Cadence switching mechanism (6h→1h) được ghi vào đâu trong Aurora PostgreSQL chưa rõ. | Open |
| I-DA-05 | ~~CROSS_SOURCE_CONFLICT~~ **Resolved** | ~~Major~~ **Resolved** | Area 1: Architecture naming | ~~UC và spec sử dụng "D1" và "HLRateLimitDO" (Cloudflare primitives) trong khi project context chuyển sang AWS.~~ **Đã resolved:** BA 2026-07-04 arc-migration update tất cả files để sử dụng AWS equivalents (D1→Aurora PostgreSQL, HLRateLimitDO→HL Rate Limiter (ElastiCache Redis), Cron scheduler→EventBridge Scheduler). | **Resolved** |
| I-DA-06 | MISSING_INFO | Medium | Area 2: Business Rules | **MỚI (2026-07-06):** OQ-EXBOT-15 (HL Rate Limiter + Redis Redlock ordering) vẫn Open trong spec.md §9. Deep-audit không sử dụng Redis Redlock nhưng FR-EXBOT-091 được thiết kế chung. | Open |
| I-DA-07 | MISSING_INFO | Low | Area 3: Functional Logic | **MỚI (2026-07-06):** UC Step 3 mismatch detection không mô tả rõ `last_known_hl_short_size` update behavior sau mismatch. | Open |
...
**Finding:** ~~UC A4 mô tả "enqueue admin escalation notification with timestamp and reason ('stop_trigger_crossed_at stuck > 30min' or 'stop_replacing_started_at stuck > 60s')" nhưng **không có nội dung message chính xác**. spec.md §5 Error Codes có E-EXBOT-008 và E-EXBOT-011 nhưng không có E-code cho hai stuck marker cases.~~ **Đã resolved:** E-EXBOT-019 và E-EXBOT-020 đã được thêm vào message-list.md (2026-07-01). UC A4 đã updated với E-code references. E-EXBOT-019: "Stop trigger marker stuck for over 30 minutes. Bot entered Safe Mode. Manual review required." E-EXBOT-020: "Stop replacement marker stuck for over 60 seconds. Bot entered Safe Mode. Manual review required."

**Impact on tester understanding:** ~~Tester không thể verify expected notification content trong test case vì message không được define. Không có E-code để trace.~~ **Resolved — Tester có thể verify notification content với E-EXBOT-019 và E-EXBOT-020.**
...
**I-DA-02 — ~~MISSING_INFO~~ **RESOLVED** (Medium)**

**Affected Area:** Area 2 (Data Object / State Attributes, Business Rules, Validations & Messages)

**Finding:** ~~UC A1 nói "update cadence to high-risk interval (1 hour)" nhưng không chỉ ra worker ghi thông tin này ở đâu. erd.md có `bots.next_deep_audit_at` nhưng không có spec entry nào nói worker update field này.~~ **Đã resolved:** Tech Lead/BA xác nhận đây là Tech Lead decision về implementation. Phase A test không bị ảnh hưởng.

**Impact on tester understanding:** ~~Tester không biết cadence switching được implement bằng cách nào — cron tự switch dựa trên current state, hay worker update next_deep_audit_at field.~~ **Resolved — Phase A test không phụ thuộc vào implementation details.**

**Suggested question/fix:** ~~Tech Lead xác nhận: (1) Worker update `bots.next_deep_audit_at` khi cadence switch, hay (2) Cron worker tự switch schedule dựa trên state query mà không cần ghi Aurora PostgreSQL.~~ **Resolved.**
...
**Finding:** UC Step 1 đọc tất cả bots nhưng không specify shard iteration strategy. spec.md NFR-EXBOT-012 đề cập Phase A = 1 shard, Phase B = 4 shards. Deep-audit UC không nói rõ worker scan tất cả shards trong một execution hay có một cron trigger per shard. BA xác nhận đây là Tech Lead decision về implementation architecture. Behavior nghiệp vụ không thay đổi dù theo cách nào.

**Impact on tester understanding:** Tester không thể design test cho multi-shard scenario (Phase B). Nếu có 4 shards, deep-audit worker cần iterate qua tất cả để audit tất cả bots. **Phase A test không bị ảnh hưởng.**

**Suggested question/fix:** Tech Lead xác nhận: Deep-audit EventBridge Scheduler trigger một worker cho tất cả shards, hay một worker per shard? Nếu per-shard, có 4 cron triggers hay 1 cron fan-out to 4 workers.

**Status:** Open

---

**I-DA-04 — ~~UNCLEAR_INFO~~ **RESOLVED** (Medium)**

**Affected Area:** Area 3 (Functional Logic & Workflow Decomposition)

**Finding:** ~~UC A2 nói "Full audit still runs at 6-hour cadence — pause does NOT skip deep-audit scheduling. All detection paths (steps 3–7) execute normally." Tuy nhiên, không có verify rõ ràng rằng stopped marker detection hoạt động cho paused bot. **Edge case:** Active bot → stop trigger fires → light-check sets `stop_trigger_crossed_at` → user pauses bot → marker stuck > 30 min trên paused bot. Deep-audit A2 path nói "all detection paths execute" nhưng UC không verify stopped marker detection được include.~~ **Đã resolved:** UC A2 đã updated: "All detection paths (steps 3–7) execute normally, including stuck marker detection (steps 4–5); if triggered, bot transitions from `paused` to `safe_mode`." states.md row 71 đã updated.

**Impact on tester understanding:** ~~Tester có thể bỏ sót test case cho edge case này. Nếu marker detection không apply cho paused bot, test case sẽ sai.~~ **Resolved — Tester biết stuck marker detection chạy cho paused bot.**

**Suggested question/fix:** ~~Tech Lead xác nhận: Deep-audit có check `stop_trigger_crossed_at` và `stop_replacing_started_at` cho paused bots không? Nếu có, UC nên make it explicit trong A2.~~ **Resolved.**
...
**Finding:** ~~UC và spec sử dụng "D1" và "HLRateLimitDO" (Cloudflare primitives) trong khi project context chuyển sang AWS architecture. Đây là cross-source conflict giữa UC/documentation và stated architecture (AWS Lambda, Aurora PostgreSQL, ElastiCache Redis).~~ **Đã resolved:** BA 2026-07-04 arc-migration update tất cả files để sử dụng AWS equivalents:
...
**I-DA-06 — MISSING_INFO (Medium)** *(NEW 2026-07-06)*

**Affected Area:** Area 2 (Data Object / State Attributes, Business Rules, Validations & Messages)

**Finding:** UC Step 2 gọi HL qua HL Rate Limiter (ElastiCache Redis) với weight=2. Tuy nhiên, OQ-EXBOT-15 trong spec.md §9 (Open Questions) vẫn chưa được resolved: "HL Rate Limiter (Redis) + User Lock (Redis Redlock) interaction in hedge-sync: should rate-limit weight be consumed before or after lock acquisition?" Deep-audit không sử dụng Redis Redlock (không có hedge mutation), nên câu hỏi này ít ảnh hưởng trực tiếp đến deep-audit behavior. Tuy nhiên, FR-EXBOT-091 được thiết kế chung cho cả hedge-sync và deep-audit, và Tech Lead decision về ordering ảnh hưởng đến system architecture tổng thể.

**Impact on tester understanding:** Tester không bị ảnh hưởng trực tiếp cho Phase A deep-audit tests (deep-audit không dùng Redis Redlock). Tuy nhiên, nếu deep-audit future-proofed để handle multi-step HL operations, ordering decision sẽ relevant.

**Suggested question/fix:** Tech Lead xác nhận OQ-EXBOT-15: HL Rate Limiter weight consumption ordering (before or after lock acquisition). Đây là Tech Lead decision về implementation.

**Status:** Open

---

**I-DA-07 — MISSING_INFO (Low)** *(NEW 2026-07-06)*

**Affected Area:** Area 3 (Functional Logic & Workflow Decomposition)

**Finding:** UC Step 3 verify actual short size từ HL matches `hedge_legs.last_known_hl_short_size`. Khi mismatch được phát hiện, UC mô tả rõ: (1) record mismatch in Aurora PostgreSQL, (2) trigger SAFE_MODE, (3) enqueue admin notification. Tuy nhiên, UC **không mô tả** liệu `last_known_hl_short_size` có được update thành actual size từ HL không, hay để nguyên giá trị mismatch. Điều này ảnh hưởng đến behavior sau khi bot recover từ SAFE_MODE. Nếu `last_known_hl_short_size` không được update, subsequent deep-audit sẽ tiếp tục phát hiện mismatch → potential loop.

**Impact on tester understanding:** Tester không biết state của `last_known_hl_short_size` sau mismatch detection để design recovery test đúng. Recovery test design phụ thuộc vào behavior này.

**Suggested question/fix:** Tech Lead xác nhận: Khi mismatch được phát hiện và bot enters SAFE_MODE, `last_known_hl_short_size` có được update thành actual size từ HL không? Hoặc đây là design decision cần BA xác nhận.

**Status:** Open

---

### Scoring Summary (Updated 2026-07-06 - Re-audit v2)

| Scoring Area | Max Points | Score v1 | Score v2 | Delta | Status |
|---|---|---|---|---|---|
| Area 1: Function / Operation & Data Object Inventory | 20 | 18/20 ✅ Clear | **18/20 ⚠️ Partial** | 0 | I-DA-03 Open |
| Area 2: Data Object / State Attributes, Business Rules, Validations & Messages | 25 | 23/25 ✅ Clear | **21/25 ⚠️ Partial** | -2 | I-DA-04, I-DA-06 Open |
| Area 3: Functional Logic & Workflow Decomposition | 25 | 23/25 ✅ Clear | **22/25 ⚠️ Partial** | -1 | I-DA-07 Open |
| Area 4: Functional Integration & Data Consistency | 15 | 13/15 ✅ Clear | **13/15 ✅ Clear** | 0 | No new issues |
| Area 5: UC / Spec Documentation Quality Issues | 15 | 13/15 ✅ Clear | **15/15 ✅ Clear** | +2 | I-DA-05 Resolved (arc-migration) |
| **Total** | **100** | **90/100** | **89/100** | **-1** | — |
| **Verdict** | — | ✅ Ready | **✅ Ready** | — | Minor changes |

**Changes from v1 to v2:**
- I-DA-05 (CROSS_SOURCE_CONFLICT: Cloudflare → AWS naming) → Resolved: BA 2026-07-04 arc-migration update all files (+2 pts Area 5)
- I-DA-06 (OQ-EXBOT-15: HL Rate Limiter ordering) → New Open issue (-1 pt Area 2)
- I-DA-07 (last_known_hl_short_size update behavior after mismatch) → New Open issue (-1 pt Area 3)
- I-DA-01, I-DA-02, I-DA-04 → Remained as in v1

**Verdict:** ✅ **Ready (89/100).** UC-EXBOT-deep-audit đã sẵn sàng cho test design sau re-audit 2026-07-06. Các issue còn lại (I-DA-03, I-DA-04, I-DA-06, I-DA-07) là Tech Lead decisions hoặc áp dụng cho Phase B, không block test design hiện tại. Điểm giảm nhẹ (-1 pt) do 2 new open questions được phát hiện trong re-audit, nhưng không ảnh hưởng đến verdict Ready.
...
*UC-EXBOT-deep-audit Readiness Review Report - v2 (2026-07-06 Re-audit after BA document update 2026-07-04)*
