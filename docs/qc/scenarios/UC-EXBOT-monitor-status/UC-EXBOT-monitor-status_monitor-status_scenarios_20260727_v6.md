...
> **Nguồn:** docs/qc/uc-read/UC-EXBOT-monitor-status/UC-EXBOT-monitor-status_monitor-status_audited_20260727_v7.md
> **Ngày tạo:** 2026-07-27
...
| E-EXBOT | Mã lỗi cấp API do ExBot Lambda trả về qua Operator Facade — quy định HTTP status code và nội dung message cho từng trường hợp lỗi. Nguồn canonical là `srs/spec.md` §5; bảng mirror `message-list.md` đã đồng bộ đầy đủ từ 2026-07-24 (V-010 Resolved). | `srs/spec.md` §5 |
| AC-ms | Acceptance Criteria candidate của UC-EXBOT-monitor-status — điều kiện chấp nhận suy ra từ UC/US, dùng làm cơ sở thiết kế scenario Acceptance. | Audit report v7 §8 |
| V-XXX | Issue ID trong Issue Register của audit report — định danh một gap/mâu thuẫn/câu hỏi mở đã được QC ghi nhận. Tại thời điểm v7, V-009/V-010/V-011/V-012 đã **Resolved** (BA xác nhận + cập nhật trực tiếp vào nguồn ngày 2026-07-24); V-001 vẫn **Open** (thiếu xác nhận chính thức bằng lời cho tên field `runtime_health_status`, dù bằng chứng kỹ thuật ERD đã đủ mạnh). | Audit report v7 §10.1 |
| isAdmin guard | Điều kiện chặn cứng trong code ExBot Lambda: nếu caller không có quyền Admin, request bị trả 403 ngay trước khi bất kỳ business logic nào của endpoint này chạy. Đây là thiếu sót đã biết của v1 (v1 gap) — đường xử lý Investor đang chờ v1.1 (dependency D2). Mâu thuẫn tài liệu giữa UC và US-EXBOT-002 về hành vi này đã được giải quyết (V-009 Resolved), nhưng bản thân hành vi chặn Investor KHÔNG thay đổi — đây vẫn là rủi ro nghiệp vụ thật trong v1. | UC §2 Preconditions, audit v7 V-009 |
...
**UC Reference:** UC-EXBOT-monitor-status — Monitor Status
**Req-ID:** UC §2 Preconditions #8; §6.1.A step 2b; AC-ms-01b; V-009 Resolved
**Test Type:** Functional
**Description:** Gửi `GET /api/exbot/status/{botId}` với chữ ký HMAC hợp lệ từ một ví Investor mà `wallet_address` khớp đúng `bot.user_wallet_address`. Vì caller không có quyền Admin, ExBot Lambda phải trả về HTTP 403 ngay tại bước 2b (isAdmin guard) — đây là hành vi thực tế của v1, đã được BA xác nhận khớp với `us-002.md` sau khi bổ sung ghi chú v1 gap vào cả 3 Acceptance Criteria (V-009 Resolved 2026-07-24), không phải bug. Mục đích scenario là ghi lại đúng hành vi hiện tại của v1 (isAdmin guard chặn trước khi đọc bản ghi bot), không phải hành vi mục tiêu của v1.1 khi guard được gỡ bỏ.
...
**UC Reference:** UC-EXBOT-monitor-status — Monitor Status
**Req-ID:** UC §6.1.A step 2b, step 3; §4 A9; AC-ms-05; V-009 Resolved
**Test Type:** Functional
**Description:** Gửi `GET /api/exbot/status/{botId}` với chữ ký HMAC hợp lệ từ ví Investor A, trong khi bot thuộc về ví Investor B (mismatch). Vì isAdmin guard (bước 2b) chạy trước bước kiểm tra ownership (bước 3, A9), request phải bị chặn ở bước 2b với HTTP 403 — cùng kết quả với scenario 004 (ví khớp). Kết hợp với scenario 004, hai scenario này tạo thành bảng quyết định xác nhận thứ tự guard: {Investor, khớp} → 403 và {Investor, không khớp} → 403 giống nhau, chứng minh tầng wallet-match (E-EXBOT-030) không thể kiểm thử độc lập bằng actor Investor trong v1. Mâu thuẫn tài liệu US/UC về hành vi này đã được BA xác nhận đóng (V-009 Resolved); hành vi chặn thực tế không đổi.
**Test Focus:** Permission/Role
...
**Scenario Title:** Permission — response body của lỗi 403 isAdmin guard đúng định dạng `{ "reason": "forbidden" }`, không có E-EXBOT code hay message text
**UC Reference:** UC-EXBOT-monitor-status — Monitor Status
**Req-ID:** UC §6.1.C; §2 Preconditions v1 gap note; V-012 Resolved
**Test Type:** Functional
**Description:** Gửi `GET /api/exbot/status/{botId}` bằng actor Investor (bị chặn ở bước 2b). Kiểm tra response trả về đúng HTTP 403 với body chính xác `{ "reason": "forbidden" }` — BA đã xác nhận từ helper `forbidden()` trên nhánh develop rằng lỗi 403 của isAdmin guard KHÔNG có E-EXBOT code và KHÔNG có message text tự do, khác với các lỗi 403 khác trong UC (ví dụ E-EXBOT-030 cho wallet-mismatch) (V-012 Resolved). Assertion phải kiểm tra cả HTTP status code = 403 và giá trị chính xác `body.reason === "forbidden"`; không được assert thêm trường nào khác không có trong body này.
...
|---|---|---|
| Toàn bộ scenario theo actor Investor (happy path, A1–A10 dưới góc nhìn Investor, kể cả A9 wallet-mismatch với dữ liệu thật) | **Mâu thuẫn tài liệu US/UC đã đóng (V-009 Resolved), nhưng bản thân hành vi vẫn BLOCKED cho mục đích kiểm thử theo D2:** isAdmin guard trong v1 chặn 403 mọi request Investor trước khi business logic chạy — không thể thiết kế test case theo actor Investor có kỳ vọng HTTP 200 hoặc bất kỳ dữ liệu status thật nào. Các scenario 004, 005, 017, 023 trong file này verify đúng hành vi 403 hiện tại của guard (đã xác nhận khớp tài liệu), nhưng không verify được nghiệp vụ Investor thật — đây vẫn là rủi ro nghiệp vụ thật đang chờ v1.1. | Chờ v1.1 gỡ isAdmin guard (D2, rủi ro nghiệp vụ độc lập với V-009 đã Resolved). Sau khi gỡ guard, thiết kế lại toàn bộ scenario theo actor Investor tương ứng với các scenario Admin hiện có (001–002, 010–018, 029–042). |
