...
> Source: docs/qc/uc-read/UC-EXBOT-bot-start/UC-EXBOT-bot-start_bot-start_audited_20260706_v3.md
> QnA: docs/qc/uc-read/UC-EXBOT-bot-start/UC-EXBOT-bot-start_bot-start_questions_20260701_v2.md
> Generated: 2026-07-01 | Updated: 2026-07-06 (v3 — arc-migration 2026-07-04: D1→Aurora PostgreSQL, ExBot Worker→ExBot Lambda, Cloudflare Queue→SQS, HLRateLimitDO→ElastiCache Redis; updated all scenario descriptions accordingly)
> Domain/Architecture: AWS Lambda (ExBot Lambda + Key-Provision Worker) + AWS KMS (Signing Lambda) + Hyperliquid perp API + BnzaExVault on-chain (Base + Optimism) + Aurora PostgreSQL (control_db + state_db_shard) + SQS (key-provision, bot-start) + ElastiCache Redis (Redlock + rate-limit)
...
**Description:** Trigger key-provision flow after an on-chain deposit event. The Key-Provision Worker must call KMS `GenerateKeyPair` twice (master key, then agent key). Verify: (1) KMS returns only the public addresses (`hl_user_address`, `agent_address`) — no private key material appears in the worker response, Aurora PostgreSQL rows, or application logs; (2) `hl_agent_keys` row is written with `key_status='active'`, `hl_user_address`, and `agent_address` populated; (3) no raw private key value is visible in any log stream.
**Test Focus:** Happy path
...
**Test Type:** Functional
**Description:** Deliver the same `key-provision` queue message twice (simulating SQS at-least-once redelivery). Verify: (1) the second delivery encounters a UNIQUE constraint conflict on `queue_idempotency.message_id` and exits immediately without calling KMS again; (2) only one `hl_agent_keys` row with `key_status='active'` exists for the user; (3) no second `bot-start` job is enqueued.
...
**Test Type:** Functional
**Description:** Trigger a `bot-start` queue message for a user who already has a bot with `status='active'` in `bot_registry`. Verify: (1) E-EXBOT-001 ("You already have an active ExBot. Close or wait for the existing bot to finish.") is returned; (2) no new bot record is created in Aurora PostgreSQL; (3) `bot_registry` count for the user remains 1.
...
**Test Type:** Functional
**Description:** Simulate HL API being unreachable when ExBot Lambda fetches `marginSummary` during preflight step 2. Verify: (1) start request fails immediately with E-EXBOT-008 ("Hyperliquid API is currently unreachable. Bot entered Safe Mode. Retrying automatically."); (2) no bot record is created in Aurora PostgreSQL; (3) preflight does not proceed to step 3. (Confirmed behavior from I-03 answer: preflight HL timeout = immediate fail, no retry loop.)
**Test Focus:** Error/Exception
...
**Test Type:** Functional
**Description:** Trigger a `bot-start` job where all preflight steps 1-4 pass but the LP mint simulation fails (e.g., pool liquidity too low for the deposit amount). Verify: (1) E-EXBOT-006 ("LP mint simulation failed. Check pool liquidity or adjust deposit amount.") is returned; (2) no bot record is created in Aurora PostgreSQL; (3) no on-chain vault call is made (simulation only, no gas consumed).
**Test Focus:** Alternative flow

---

### Scenario ID: TS_UC-EXBOT-bot-start_019
**Scenario Title:** Preflight failure — no partial bot record left in Aurora PostgreSQL after any preflight step fails
**UC Reference:** UC-EXBOT-bot-start
**Req-ID:** FR-EXBOT-002, FR-EXBOT-003
**Test Type:** Functional
**Description:** For each of the 5 preflight steps individually (steps 1-5), trigger a failure at that step. After each failure, query Aurora PostgreSQL `bot_registry` and verify zero new records exist for the user. The bot record must only be created AFTER all 5 preflight checks pass (UC §3 step 4). No partial records under any failure scenario.
**Test Focus:** Error/Exception
...
**Req-ID:** FR-EXBOT-003
**Test Type:** Data/State
**Description:** After all 5 preflight checks pass, verify that a bot record is created in Aurora PostgreSQL with `lifecycle_state='preflight'` and `status` set to the appropriate coarse-grained value, in a single atomic operation. The record must be present in Aurora PostgreSQL before any `vaultMint` on-chain call is made. Query Aurora PostgreSQL between preflight completion and vault call to confirm.
**Test Focus:** State transition
...
**Test Type:** Data/State
**Description:** After the bot record is created with `lifecycle_state='preflight'`, ExBot Lambda calls `BnzaExVault.vaultMint(...)`. When the `VaultMinted(user, botId, tokenId, liquidity)` on-chain event is received, verify: (1) `positions.token_id` is populated with the correct `tokenId` from the event; (2) `positions.custodian='vault'`; (3) `positions.tick_lower`, `positions.tick_upper` are stored; (4) `bots.lifecycle_state='lp_opened'`.
...
**Test Type:** Functional
**Description:** After `lp_opened` state, trigger hedge computation. Verify that `targetShortEth = lpEthAmount × 0.70` is computed using BigDecimal arithmetic. Specifically: (1) no intermediate `Number()` conversion is used for `lpEthAmount` or `hedgeRatio`; (2) the result stored in Aurora PostgreSQL is a TEXT string (not a JSON number); (3) `normalizeTargetRatioBps("0.70")` returns 7000n (not 6999n or 7001n due to float rounding).
**Test Focus:** Boundary
...
**Req-ID:** FR-EXBOT-080, NFR-EXBOT-006
**Test Type:** Integration
**Description:** After `targetShortEth` is computed, ExBot Lambda sends the signing request to Signing Lambda. Verify: (1) Signing Lambda calls KMS `kms:Sign` using the agent key (not the master key); (2) the signed IOC order is sent to HL; (3) the Signing Lambda is the only IAM principal that calls `kms:Sign`; (4) no private key material appears in the signed payload or in ExBot Lambda memory/logs.
**Test Focus:** Integration
...
**Test Type:** Functional
**Description:** Simulate HL API being unreachable when ExBot Lambda sends the short IOC order (after `lp_opened`). Verify: (1) E-EXBOT-008 is surfaced; (2) the LP NFT is still held by BnzaExVault (the LP was already minted — no hedge placed); (3) no stop is placed in this state. Note (I-11 OPEN): UC §4 A7 records `lifecycle_state='error'` but `srs/states.md` state diagram shows `hedge_pre_open → safe_mode` as the failure transition — the destination state is in conflict. This scenario verifies the failure gate and LP-open-without-hedge condition; expected destination state is ambiguous pending BA confirmation (I-11).
**Test Focus:** Error/Exception
...
**Req-ID:** UC §4 A10
**Test Type:** Functional
**Description:** Simulate HL accepting the signed request but rejecting the IOC order itself (e.g., margin insufficient at order time, self-trade, or order parameter error). Verify: (1) HL rejection reason is captured; (2) no hedge position exists on HL after the failure. Note (I-11 OPEN): UC §4 A10 records `lifecycle_state='error'` but `srs/states.md` shows `hedge_pre_open → safe_mode` as the failure transition — the destination state is in conflict. This scenario verifies the rejection-detected condition; expected destination state is ambiguous pending BA confirmation (I-11).
**Test Focus:** Error/Exception
...
**Test Type:** Data/State
**Description:** After `stop_trigger_px` is computed, ExBot Lambda places a reduce-only stop market via Signing Lambda. Verify: (1) the stop is placed with the correct trigger price and size; (2) HL confirms the stop order; (3) `hedge_legs.stop_cloid` and `hedge_legs.stop_price` are written; (4) `bots.lifecycle_state` transitions to `'stop_verified'` then immediately to `'active'`; (5) the bot cannot reach `active` state without a confirmed stop — the INV-STOP invariant is satisfied from initialization.
**Test Focus:** State transition

---

### Scenario ID: TS_UC-EXBOT-bot-start_035
**Scenario Title:** Stop placement failure — lifecycle transitions to safe_mode per states.md (stop_placing → safe_mode) and E-EXBOT-009 is surfaced (A8)
**UC Reference:** UC-EXBOT-bot-start
**Req-ID:** FR-EXBOT-031, UC §4 A8, E-EXBOT-009, FR-EXBOT-050
**Test Type:** Functional
**Description:** Simulate HL rejecting or failing to confirm the reduce-only stop market after the short IOC was already filled. Verify: (1) E-EXBOT-009 ("Failed to place native stop on Hyperliquid. Bot cannot activate without a stop.") is triggered; (2) an operator alert is sent; (3) the bot does NOT reach `lifecycle_state='active'`; (4) the HL short position remains open; (5) `bots.lifecycle_state` transitions to `'safe_mode'` per `srs/states.md` (`stop_placing → safe_mode`) — confirmed by BA (qc-notes-temp.md, I-06, 2026-07-02); (6) auto-recovery per FR-EXBOT-050 begins: retry when HL is responsive and 3 reconciles succeed; if irrecoverable → `bot_safe_close` is triggered.
**Test Focus:** Error/Exception
...
**Scenario Title:** Lifecycle state machine — hedge_pre_open to safe_mode transition when HL short fails (I-11: UC vs states.md conflict open)
**UC Reference:** UC-EXBOT-bot-start
**Req-ID:** FR-EXBOT-003, srs/states.md
**Test Type:** Data/State
**Description:** Simulate HL short IOC failure during the `hedge_pre_open` → `hedge_post_confirmed` transition (A7 or A10). According to `srs/states.md`, the valid transitions from `hedge_pre_open` are: → `hedge_post_confirmed` (success) or → `safe_mode` (HL order fails). Verify that on IOC failure, the lifecycle state transitions to `safe_mode` per `srs/states.md`. Note (I-11 OPEN): UC §4 A7/A10 records `error` instead of `safe_mode` — this conflict is tracked as I-11 (open blocker, pending BA confirmation). `srs/states.md` is treated as the authoritative state machine per audit v2; this scenario asserts the states.md behavior. Expected destination state must be re-confirmed once I-11 is resolved.
**Test Focus:** State transition
...
**Test Type:** Functional
**Description:** Deliver the same `bot-start` queue message twice (redelivery scenario). Verify: (1) the second delivery encounters a UNIQUE constraint conflict on `queue_idempotency.message_id` and exits immediately; (2) only one bot record exists in Aurora PostgreSQL for the user; (3) no duplicate HL orders are submitted; (4) no duplicate `positions` or `hedge_legs` rows are created.
**Test Focus:** Idempotency/Concurrency
...
**Req-ID:** FR-EXBOT-080, NFR-EXBOT-006
**Test Type:** Functional
**Description:** After key-provision completes and bot-start runs, dump the `hl_agent_keys` table from Aurora PostgreSQL. Verify: (1) all rows contain only public addresses (`hl_user_address`, `agent_address`) and metadata; (2) no field contains a hex-encoded or base64-encoded 256-bit/512-bit private key pattern; (3) application logs searched for the time window of provisioning show no raw key values. Private keys must remain exclusively within AWS KMS HSM.
**Test Focus:** Happy path

---

### Scenario ID: TS_UC-EXBOT-bot-start_042
**Scenario Title:** Security — direct HTTP request to ExBot Lambda returns 403 (not publicly accessible)
**UC Reference:** UC-EXBOT-bot-start
**Req-ID:** FR-EXBOT-090, BR-EXBOT-010
**Test Type:** Functional
**Description:** Send a direct invocation request to ExBot Lambda (bypassing the OPERATOR Facade / API Gateway). Verify the Lambda returns 403 and does not process the request. All bot-start operations must only be accessible via the OPERATOR Facade at `POST /api/exbot/start`, which proxies via API Gateway + HMAC Lambda Authorizer (per F-05 flows.md, BR-EXBOT-010: standalone Lambda — not publicly exposed).
...
**Test Type:** Integration
**Description:** After bot-start completes, query on-chain BnzaExVault for the LP NFT held for the user. Verify `positions.token_id` in Aurora PostgreSQL matches the actual `tokenId` from the `VaultMinted` event and that the NFT is currently held by BnzaExVault (`custodian='vault'`). This consistency is critical for future `user_redeem` and `bot_safe_close` flows that call `BnzaExVault.redeem(tokenId)`.
**Test Focus:** On-chain↔off-chain consistency
...
**Test Type:** Acceptance
**Description:** For each of the 5 preflight failure scenarios (one-bot policy, margin insufficient, key not provisioned, builder fee missing, simulation fail), verify that zero bot records exist in Aurora PostgreSQL for the user after the failure. The creation of any partial bot record is a defect per FR-EXBOT-002.
**Test Focus:** Alternative flow
...
|---|---|---|
| A4 postcondition — on-chain fund return mechanism after LP mint failure | BLOCKED: BnzaExVault ABI not confirmed (OQ-EXBOT-08 Open; IC-EXBOT-002). Cannot determine if revert is automatic or requires explicit ExBot Lambda action. I-05 Deferred. | Resolve OQ-EXBOT-08 → confirm ABI → add specific postcondition check to scenario TS_022 |
...
| A7/A10 expected destination state — error vs safe_mode conflict at hedge step | OPEN ISSUE I-11: UC §4 A7/A10 records `error` but `srs/states.md` shows `hedge_pre_open → safe_mode`. Scenarios TS_028, TS_029, and TS_038 flag this conflict; TS_038 asserts states.md behavior pending BA confirmation. | BA/zen to confirm which is authoritative → update TS_028/029 expected destination state; revise TS_038 note if states.md confirmed |
