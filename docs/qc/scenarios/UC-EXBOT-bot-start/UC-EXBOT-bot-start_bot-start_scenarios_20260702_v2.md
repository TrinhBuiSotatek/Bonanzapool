...
> Source: docs/qc/uc-read/UC-EXBOT-bot-start/UC-EXBOT-bot-start_bot-start_audited_20260702_v2.md
> QnA: docs/qc/uc-read/UC-EXBOT-bot-start/UC-EXBOT-bot-start_bot-start_questions_20260701_v2.md
> Generated: 2026-07-01 | Updated: 2026-07-02 (v2 — I-06 resolved: TS_035 confirmed safe_mode; I-11 flagged on TS_028/029/038)
...
**Test Type:** Functional
**Description:** Simulate HL API being unreachable when ExBot Worker sends the short IOC order (after `lp_opened`). Verify: (1) E-EXBOT-008 is surfaced; (2) the LP NFT is still held by BnzaExVault (the LP was already minted — no hedge placed); (3) no stop is placed in this state. Note (I-11 OPEN): UC §4 A7 records `lifecycle_state='error'` but `srs/states.md` state diagram shows `hedge_pre_open → safe_mode` as the failure transition — the destination state is in conflict. This scenario verifies the failure gate and LP-open-without-hedge condition; expected destination state is ambiguous pending BA confirmation (I-11).
**Test Focus:** Error/Exception
