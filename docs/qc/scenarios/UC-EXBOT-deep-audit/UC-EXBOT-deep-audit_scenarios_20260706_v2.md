...
> **Source:** `docs/qc/uc-read/UC-EXBOT-deep-audit/UC-EXBOT-deep-audit_audited_20260630_v2.md` (v2)
> **Generated:** 2026-07-06
> **Updated:** 2026-07-06 (v2 — after BA arc-migration 2026-07-04)
> **Domain/Architecture:** AWS Lambda EventBridge Scheduler-triggered worker — no UI. Deep-Audit Worker reads Aurora PostgreSQL, calls Hyperliquid API via HL Rate Limiter (ElastiCache Redis) with weight=2, updates Aurora PostgreSQL, and enqueues notifications. Aurora PostgreSQL shard topology: Phase A = 1 shard, Phase B = 4 shards (shard iteration strategy pending Tech Lead decision per Q-DA-03).
...
**Description:** EventBridge Scheduler triggers deep-audit for a bot with status='active', no hedge size mismatch, no stuck markers, and HL API is reachable. Worker reads bot state from Aurora PostgreSQL, calls HL clearinghouseState via HL Rate Limiter (ElastiCache Redis) (weight=2), verifies short size matches last_known_hl_short_size, checks stuck markers (both NULL or not aged), fetches marginSummary, updates hedge_legs.margin_status, sets last_audit_at=now, and inserts queue_idempotency state='succeeded'.
...
**Test Type:** Functional
**Description:** Deep-audit worker calls HL clearinghouseState via HL Rate Limiter (ElastiCache Redis) and finds actual short size ≠ hedge_legs.last_known_hl_short_size. Worker records the mismatch in Aurora PostgreSQL with the size delta (actual − expected), triggers bot transition to safe_mode, and enqueues admin notification with message E-EXBOT-011: "Hedge position mismatch detected. Bot entered Safe Mode pending reconciliation." Note: Behavior of last_known_hl_short_size after mismatch detection (update to actual or keep mismatch value) is pending Tech Lead decision per Q-DA-07.
**Test Focus:** Error/Exception — reconciliation failure leading to SAFE_MODE
**Open Question:** Q-DA-07 — last_known_hl_short_size update behavior after mismatch
...
**Description:** HL Rate Limiter (ElastiCache Redis) returns {allowed: false} OR HL API returns 5xx when deep-audit worker attempts to call clearinghouseState. Worker skips Steps 2, 3, 4, 6, 7 (HL-dependent steps), switches cadence to high-risk interval (1 hour), enqueues notification "Hyperliquid API unreachable" with message E-EXBOT-008, and records retry-pending state in Aurora PostgreSQL. Bot remains in its current state.
...
**Test Type:** Functional
**Description:** A bot has status='paused'. EventBridge Scheduler triggers deep-audit and worker reads the bot's state. Worker executes all detection paths (Steps 3–7) normally: clearinghouseState reconciliation, stop_trigger_crossed_at stuck check (>30min), stop_replacing_started_at stuck check (>60s), marginSummary fetch, and last_audit_at update. Pause does NOT skip deep-audit scheduling or any detection path.
...
**Test Type:** Functional
**Description:** A bot has circuit_breakers.state='open'. Deep-audit worker detects this condition before processing. The EventBridge Scheduler schedule for this bot switches from the normal 6-hour interval to the high-risk 1-hour interval. Subsequent deep-audit runs for this bot occur every 1 hour until circuit_breakers.state returns to 'closed'. Note: Implementation of cadence switching mechanism (worker updates bots.next_deep_audit_at or scheduler self-switches based on state query) is pending Tech Lead decision per Q-DA-04.
**Test Focus:** Alternative flow — cadence switching based on circuit breaker state
**Open Question:** Q-DA-04 — cadence switching mechanism implementation
...
**Test Type:** Functional
**Description:** A bot has hedge_legs.margin_status='warning'. Deep-audit worker detects this condition. The EventBridge Scheduler schedule switches from 6-hour to 1-hour interval. All subsequent deep-audit runs for this bot occur at 1-hour cadence. Note: margin_status update in Step 6 may change the status — if it transitions from warning to ok, cadence may revert. Margin threshold values (0.55/0.75) are subject to Phase 0 backtest finalization (OQ-EXBOT-06).
**Test Focus:** Alternative flow — cadence switching based on margin status
**Open Question:** OQ-EXBOT-06 — margin threshold values pending Phase 0 backtest
...
**Req-ID:** AC-DA-07; FR-EXBOT-060
**Test Type:** Functional
**Description:** A bot has hedge_legs.margin_status='critical'. Deep-audit worker detects this condition. The EventBridge Scheduler schedule switches from 6-hour to 1-hour interval. Deep-audit processes the bot at high-risk cadence going forward. Note: two consecutive critical margin status values trigger SAFE_MODE per FR-EXBOT-050, but deep-audit only updates margin_status — the consecutive-count logic is handled elsewhere.
...
**Req-ID:** FR-EXBOT-016
**Test Type:** Functional
**Description:** A bot has circuit_breakers.state='closed' and hedge_legs.margin_status='ok'. Deep-audit worker verifies both conditions. The EventBridge Scheduler schedule remains at the normal 6-hour interval. No cadence switching occurs. Deep-audit processes the bot at normal cadence.
...
**Scenario Title:** HL Rate Limiter (ElastiCache Redis) integrated — deep-audit calls HL with weight=2
**UC Reference:** UC-EXBOT-deep-audit §3 Step 2; FR-EXBOT-091
**Req-ID:** AC-DA-09; FR-EXBOT-091; BR-EXBOT-003
**Test Type:** Integration
**Description:** Deep-audit worker calls HL clearinghouseState via HL Rate Limiter (ElastiCache Redis) with declared weight=2. The rate limiter accepts the weight call since it is within the 800 weight/min budget (67% of HL's 1,200 weight/min hard limit). Worker proceeds with the HL API call. BR-EXBOT-003 (HL weight=0) does NOT apply to deep-audit — this is the exception confirmed in UC §6 BR. Note: HL Rate Limiter + User Lock interaction ordering (before or after lock acquisition) is pending Tech Lead decision per OQ-EXBOT-15 and Q-DA-06. Deep-audit does not use Redis Redlock (no hedge mutation).
**Test Focus:** Integration — HL rate limit compliance
**Open Question:** Q-DA-06 / OQ-EXBOT-15 — HL Rate Limiter + User Lock ordering
...
**Test Type:** Functional
**Description:** EventBridge Scheduler triggers deep-audit. Worker reads all bots with status IN ('active', 'paused') from Aurora PostgreSQL. Bots with status='closed', 'error', 'idle', 'lp_rebalancing', 'lp_closing', or other non-active/paused statuses are NOT included in the audit scope (per states.md registry). The worker processes zero bots in these categories and completes successfully.
...
**Scenario Title:** Deep-audit reads from correct Aurora PostgreSQL shard — bot state and hedge_legs joined correctly
**UC Reference:** UC-EXBOT-deep-audit §3 Step 1; erd.md
**Req-ID:** FR-EXBOT-016; NFR-EXBOT-012
**Test Type:** Integration
**Description:** Deep-audit worker reads bots.status, bots.lifecycle_state, hedge_legs, circuit_breakers.state, and margin_status from Aurora PostgreSQL. Worker correctly joins hedge_legs to bots on bot_id and reads circuit_breakers.state from the circuit_breakers table. All fields are present and valid before proceeding to Step 2. Note: Phase A = 1 shard (no sharding). Phase B multi-shard iteration strategy is pending Tech Lead decision (Q-DA-03).
**Test Focus:** Integration — Aurora PostgreSQL read correctness
**Open Question:** Q-DA-03 — multi-shard iteration strategy for Phase B
...
**Req-ID:** FR-EXBOT-016
**Test Type:** Functional
**Description:** EventBridge Scheduler triggers deep-audit but no bot in Aurora PostgreSQL has status='active' or status='paused'. Worker queries Aurora PostgreSQL and receives an empty result set. Worker completes the cron cycle without error — no HL calls are made, no Aurora PostgreSQL writes occur, and no notifications are enqueued. The queue_idempotency row is still inserted to mark the audit cycle occurred.
...
### Scenario ID: TS_EXBOT-deep-audit_029 (NEW)
**Scenario Title:** HL Rate Limiter (ElastiCache Redis) budget enforcement — weight exceeds remaining budget
**UC Reference:** UC-EXBOT-deep-audit §3 Step 2 + A1; FR-EXBOT-091
**Req-ID:** FR-EXBOT-091; AC-DA-09
**Test Type:** Integration
**Description:** Deep-audit worker attempts to call HL clearinghouseState via HL Rate Limiter (ElastiCache Redis) with weight=2, but the remaining budget in the current 60-second window is less than 2. Rate Limiter returns {allowed: false, retryAfterMs}. Worker follows A1 path: skips Steps 2, 3, 4, 6, 7, switches cadence to 1h, and enqueues notification E-EXBOT-008 "Hyperliquid API unreachable". Note: OQ-EXBOT-15 (Tech Lead) determines if weight consumption happens before or after Redis Redlock acquisition — this is less relevant for deep-audit as it does not use Redis Redlock.
**Test Focus:** Integration — rate limit exceeded
**Open Question:** Q-DA-06 / OQ-EXBOT-15 — weight consumption ordering

---

### Scenario ID: TS_EXBOT-deep-audit_030 (NEW)
**Scenario Title:** Reconciliation mismatch — last_known_hl_short_size state after SAFE_MODE entry
**UC Reference:** UC-EXBOT-deep-audit §3 Step 3 + A3; Q-DA-07
**Req-ID:** FR-EXBOT-016; FR-EXBOT-050; Q-DA-07
**Test Type:** Data/State
**Description:** Deep-audit worker detects reconciliation mismatch: actual short size on HL ≠ hedge_legs.last_known_hl_short_size. Worker records mismatch in Aurora PostgreSQL and triggers SAFE_MODE entry. The question is: does the worker update last_known_hl_short_size to the actual HL size, or does it keep the mismatched value? If not updated, subsequent deep-audit runs will continue to detect the same mismatch, potentially creating a loop. This behavior affects recovery test design. Pending Tech Lead decision per Q-DA-07.
**Test Focus:** Data consistency — mismatch state persistence
**Open Question:** Q-DA-07 — last_known_hl_short_size update behavior after mismatch
...
| Cadence switching mechanism — where 1h is recorded in Aurora PostgreSQL | Not specified in UC whether worker updates bots.next_deep_audit_at or EventBridge Scheduler self-switches based on state (Q-DA-04). Implementation detail. | Tech Lead to confirm; test design does not depend on this detail |
...
| HL Rate Limiter + User Lock ordering (OQ-EXBOT-15) | Deep-audit does not use Redis Redlock; less relevant for deep-audit behavior | Tech Lead decision for hedge-sync (separate UC) |
...
| Test Type | Count |
|---|---|
| Functional | 16 scenarios |
| Integration | 4 scenarios |
| Data/State | 6 scenarios |
...
| AC-DA-02 | TS_EXBOT-deep-audit_002, TS_EXBOT-deep-audit_030 | Mismatch → SAFE_MODE; Q-DA-07 behavior |
...
| AC-DA-09 | TS_EXBOT-deep-audit_021, TS_EXBOT-deep-audit_029 | HL Rate Limiter integrated |

### Notes
- **Inferred UC IDs / function names:** None — UC-EXBOT-deep-audit is explicitly named in source documents.
- **Blocked coverage cells:** Multi-shard iteration (Phase B) — blocked by Q-DA-03 (Tech Lead decision). Cadence-switch persistence mechanism (Q-DA-04) — implementation detail, does not block test design. last_known_hl_short_size behavior after mismatch (Q-DA-07) — Tech Lead decision needed.
- **Out-of-scope items flagged:** Multi-shard Phase B (Q-DA-03), cadence-switch Aurora PostgreSQL write mechanism (Q-DA-04), SAFE_MODE auto-recovery, light-check primary detection, INV-STOP internals, HL Rate Limiter + User Lock ordering (OQ-EXBOT-15 — hedge-sync decision), performance/load testing.
- **Changes from v1:** Updated architecture naming (Cloudflare → AWS: D1 → Aurora PostgreSQL, HLRateLimitDO → HL Rate Limiter (ElastiCache Redis), Cron → EventBridge Scheduler). Added new scenarios TS_029 (rate limit exceeded), TS_030 (mismatch state persistence). Updated scenarios to reflect resolved issues (E-EXBOT-019/020, paused bot stuck marker).
...
| v2 | 2026-07-06 | Updated after BA arc-migration (2026-07-04): Cloudflare → AWS naming. Added TS_029, TS_030 for new open questions. Updated all scenarios to reflect AWS architecture. |

---

*Test Scenarios — UC-EXBOT-deep-audit v2 (2026-07-06)*
