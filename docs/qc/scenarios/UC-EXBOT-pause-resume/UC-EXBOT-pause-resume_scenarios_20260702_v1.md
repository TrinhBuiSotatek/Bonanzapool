...
> Source: `docs/qc/uc-read/UC-EXBOT-pause-resume/UC-EXBOT-pause-resume_pause-resume_audited_20260630_v1.md` (v3, 2026-07-03)
...
**UC Reference:** UC-EXBOT-pause-resume §3 (v2026-07-03)
**Req-ID:** FR-EXBOT-005; AC-01; AC-08; BR-EXBOT-002
**Test Type:** Functional
**Description:** Investor sends `POST /api/exbot/pause` with `botId` for a bot with `bots.status='active'` and `bots.lifecycle_state='active'`. The ExBot Worker (synchronous HTTP handler, no queue) verifies preconditions, sets `bots.status='paused'` with conditional UPDATE (`WHERE status='active'`), persists atomically to D1, and returns API response `{ status: "paused", message: "Bot paused. Hedge and LP are maintained." }` with MSG-SUC-81. After pause, `bots.lifecycle_state` remains `'active'`, `hedge_legs.last_known_hl_short_size` is unchanged, and `positions.token_id` is unchanged. Audit log records "Bot paused by investor".
...
**UC Reference:** UC-EXBOT-pause-resume §5 A1 (v2026-07-03)
**Req-ID:** FR-EXBOT-050; AC-03
**Test Type:** Functional
**Description:** Investor sends `POST /api/exbot/pause` for a bot with `bots.status='safe_mode'`. The ExBot Worker returns HTTP 409 with message: "Bot is in Safe Mode. You can close the bot instead." (verbatim E-EXBOT-013 from message-list.md). No state change occurs in D1.
...
**UC Reference:** UC-EXBOT-pause-resume §5 A3 (v2026-07-03)
**Req-ID:** FR-EXBOT-005; AC-04
**Test Type:** Functional
**Description:** Investor sends `POST /api/exbot/pause` for a bot already with `bots.status='paused'`. The ExBot Worker returns a success response with message "Bot already paused. No change needed." No D1 record is modified. This idempotent behavior is enforced by the conditional UPDATE (`WHERE status='active'`) — the request that finds `status='paused'` does not update anything.
...
**UC Reference:** UC-EXBOT-pause-resume §5 A2 (v2026-07-03)
**Req-ID:** FR-EXBOT-005; AC-05
**Test Type:** Functional
**Description:** Investor sends `POST /api/exbot/resume` for a bot already with `bots.status='active'`. The ExBot Worker returns a success response with message "Bot already active. No change needed." No D1 record is modified. This idempotent behavior is enforced by the conditional UPDATE (`WHERE status='paused'`) — the request that finds `status='active'` does not update anything.
**Test Focus:** Idempotency/Concurrency
**Note:** Q3 resolved — race condition prevented by DB conditional UPDATE (BA response 2026-07-03).
...
**UC Reference:** UC-EXBOT-pause-resume §7 Postconditions (v2026-07-03)
**Req-ID:** FR-EXBOT-005; AC-08
**Test Type:** Functional
**Description:** Investor successfully pauses an active bot. The ExBot Worker records an audit log entry with content "Bot paused by investor" and a timestamp in D1. Audit log contains bot_id, user_wallet_address, timestamp, and action type.
**Test Focus:** Happy path
...
**Test Type:** Functional
**Description:** Investor successfully resumes a paused bot. The ExBot Worker records an audit log entry with content "Bot resumed by investor" and a timestamp in D1. Audit log contains bot_id, user_wallet_address, timestamp, and action type.
**Test Focus:** Happy path
...
**UC Reference:** UC-EXBOT-pause-resume §3 step 5 (v2026-07-03)
...
**UC Reference:** UC-EXBOT-pause-resume §3 step 6 (v2026-07-03)
...
**UC Reference:** UC-EXBOT-pause-resume §6 (v2026-07-03)
...
**UC Reference:** UC-EXBOT-pause-resume §6; states.md (v2026-07-03)
...
**UC Reference:** UC-EXBOT-pause-resume §1 (v2026-07-03); FR-EXBOT-001
**Req-ID:** FR-EXBOT-001
**Test Type:** Functional
...
**UC Reference:** UC-EXBOT-pause-resume §2 (v2026-07-03)
**Req-ID:** FR-EXBOT-005
**Test Type:** Functional
...
**Description:** Investor sends two simultaneous `POST /api/exbot/pause` requests for the same active bot. The ExBot Worker (synchronous handler, no queue) processes exactly one state change to `bots.status='paused'` via conditional UPDATE `WHERE status='active'`. The second request finds `bots.status='paused'` and receives the idempotent success response "Bot already paused. No change needed." — no error, no partial state, no race condition in D1. Each request gets a response; only one D1 update occurs.
**Test Focus:** Idempotency/Concurrency
**Note:** Q2, Q3 resolved — Pause/resume handled by ExBot Worker synchronous HTTP handler (not queue consumer). Race condition prevented by DB conditional UPDATE (BA response 2026-07-03).

---

### Scenario ID: TS_EXBOT_PAUSE_021
**Scenario Title:** Concurrent pause and resume requests for the same bot are handled safely
**UC Reference:** UC-EXBOT-pause-resume §5 A2, A3 (v2026-07-03)
**Req-ID:** FR-EXBOT-005
**Test Type:** Idempotency/Concurrency
**Description:** Investor sends a pause request and a resume request for the same bot simultaneously. The ExBot Worker processes exactly one state change via conditional UPDATE — whichever request runs first wins (`WHERE status='active'` for pause wins if bot was active, `WHERE status='paused'` for resume wins if bot was paused). The losing request receives its idempotent success response ("Bot already paused/active. No change needed."). Final `bots.status` is deterministic (whichever condition matched first), and both operations complete successfully with appropriate responses.
**Test Focus:** Idempotency/Concurrency
...
**UC Reference:** UC-EXBOT-pause-resume §4 step 5 (v2026-07-03)
**Req-ID:** FR-EXBOT-013; AC-02
**Test Type:** Functional
**Description:** Bot has `bots.status='paused'` with `bot_runtime_state.next_light_check_at` set far in the past (expired schedule). Investor resumes the bot. The ExBot Worker sets `next_light_check_at = now + 5min + jitter(−45s, +45s)`. The jitter value is verified to be within the range [now + 4m15s, now + 5m45s]. A `light-check` message is enqueued. Same jitter formula applies to resume scheduling as to normal scheduling per FR-EXBOT-013.
**Test Focus:** Happy path
...
**UC Reference:** UC-EXBOT-pause-resume §3–4 (v2026-07-03); FR-EXBOT-012
**Req-ID:** BR-EXBOT-003; FR-EXBOT-012
**Test Type:** Integration
**Description:** Investor pauses an active bot. During the pause operation, the ExBot Worker makes zero calls to the Hyperliquid API. No `hedge_legs` fields are updated from HL. No HL order submission. The pause operation relies solely on D1 state. HL read/write is not part of the pause flow.
...
**UC Reference:** UC-EXBOT-pause-resume §4 (v2026-07-03); BR-EXBOT-003
**Req-ID:** BR-EXBOT-003
**Test Type:** Integration
**Description:** Investor resumes a paused bot. During the resume operation, the ExBot Worker makes zero calls to the Hyperliquid API. No HL position fetch, no order placement, no margin check against HL. The `light-check` message is enqueued, but light-check itself is a separate worker that makes no HL calls per BR-EXBOT-003. Resume only updates D1 and enqueues light-check.
**Test Focus:** Integration
...
**UC Reference:** UC-EXBOT-pause-resume §3 step 3, §4 step 3 (v2026-07-03)
**Req-ID:** FR-EXBOT-005
**Test Type:** Data/State
**Description:** Investor sends a pause request. The ExBot Worker starts the D1 conditional UPDATE (`bots.status='paused' WHERE status='active'`) but crashes mid-transaction (e.g., network timeout, worker SIGKILL). The D1 transaction must be atomic — if the worker crashes before commit, D1 must remain at the previous consistent state (`bots.status='active'`) with no partial write. No inconsistent `bots.status` value exists in the database.
...
**UC Reference:** UC-EXBOT-pause-resume §6; FR-EXBOT-050; FR-EXBOT-016 (v2026-07-03)
...
**UC Reference:** UC-EXBOT-pause-resume §6; FR-EXBOT-012 (v2026-07-03)
**Req-ID:** FR-EXBOT-012
**Test Type:** Data/State
...
**UC Reference:** UC-EXBOT-pause-resume §6; FR-EXBOT-033; FR-EXBOT-050 (v2026-07-03)
**Req-ID:** FR-EXBOT-033
**Test Type:** Functional
...
**UC Reference:** UC-EXBOT-pause-resume §3–4 (v2026-07-03)
**Req-ID:** FR-EXBOT-005
**Test Type:** Data/State
...
**UC Reference:** UC-EXBOT-pause-resume §3 step 7, §4 step 6 (v2026-07-03); message-list.md (v2026-07-03)
**Req-ID:** FR-EXBOT-005
...
**UC Reference:** UC-EXBOT-pause-resume §1 (v2026-07-03); spec.md §6
**Req-ID:** FR-EXBOT-005
**Test Type:** Acceptance
...
*Test Scenarios — UC-EXBOT-pause-resume (Pause and Resume Bot) — 31 scenarios — generated by qc-func-scenario-design-exbot skill (2026-07-02, updated 2026-07-03)*
