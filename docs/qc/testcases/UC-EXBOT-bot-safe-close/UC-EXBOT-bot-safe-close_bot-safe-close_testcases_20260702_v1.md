...
> **Scenarios:** UC-EXBOT-bot-safe-close_bot-safe-close_scenarios_20260702_v1.md
...
| TC_009 | Verify hedge close order submitted with deterministic cloid | Bot với existing HL short position, Close Worker acquire UserLockDO lease (Q7: Tech Lead xác nhận requirement này) | Gọi `closeShortReduceOnlyIoc(size, cloid)` với deterministic cloid = keccak256("bnza:{botId}:{attemptId}:{stage}:{version}") | 1. `closeShortReduceOnlyIoc` được submit với đúng deterministic cloid. 2. HL có thể deduplicate trên cùng cloid; không tạo duplicate order. **Note:** Phụ thuộc Q7 - Tech Lead xác nhận UserLockDO acquire có bắt buộc cho Close Worker hay không. | Medium |
...
| TC ID | Test Title/Summary of test cases | Pre-conditions | Test Steps | Expected Result | Priority |
|---|---|---|---|---|---|
| TC_020 | Verify duplicate trigger via queue redelivery is rejected (queue idempotency) | bot_safe_close trigger message đã được xử lý thành công | 1. Message được redeliver (cùng idempotency_key, ví dụ: consumer ack failure). 2. Quan sát hệ thống response | 1. Duplicate bị reject qua UNIQUE constraint. 2. Đúng một `close_operations` row tồn tại; operation không bị double-apply. **Note:** Phụ thuộc Q4 - Tech Lead xác nhận Close Worker trigger mechanism (queue message vs direct call). Nếu là queue mechanism thì mới có queue redelivery scenario. | High |
...
| TC ID | Test Title/Summary of test cases | Pre-conditions | Test Steps | Expected Result | Priority |
|---|---|---|---|---|---|
| TC_029 | Verify admin force-close initiates full bot_safe_close flow | Admin được authenticate, bot với `bots.status='active'` | 1. Admin gọi POST /api/exbot/close với valid botId và reason. 2. Verify request forwarded qua OperatorFacade → CF service binding → ExBot Worker. 3. Verify `bots.status='closing'`. 4. Verify full flow executes (hedge → LP → RedemptionQueue) | 1. bot_safe_close được khởi tạo. 2. Investor notified: "Your ExBot was administratively closed. USDC is available for withdrawal." | High |
...
| TC ID | Test Title/Summary of test cases | Pre-conditions | Test Steps | Expected Result | Priority |
|---|---|---|---|---|---|
| TC_036 | Verify force-close requires valid admin authentication | Request không có valid OPERATOR facade token | Gọi POST /api/exbot/close mà không có valid auth token | 1. Request bị reject tại facade level. 2. Trả về 403 Forbidden. 3. Request không được forward đến ExBot Worker | High |
...
|---|---|---|---|
| UserLockDO TTL expiry during hedge close | **Q7 pending Tech Lead:** UC §3 vs FR-EXBOT-072 không xác nhận Close Worker có cần acquire UserLockDO không. FR-EXBOT-026, 092 chỉ specify cho hedge-sync. | Confirm Tech Lead Q7 trước khi execute | TC_001, TC_003, TC_009 |
| Close Worker trigger mechanism: queue vs direct call | **Q4 pending Tech Lead:** UC không đề cập queue idempotency cho Close Worker. flows.md F-05 diagram simplified. | Confirm Tech Lead Q4 trước khi execute | TC_003, TC_020 |
